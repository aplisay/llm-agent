/**
 * Provision the least-privilege analysis-service identity for the polite-ai
 * post-call analysis seam. Idempotently creates a synthetic user with role
 * `analysisService` (agent:invoke plus the cross-tenant agent:readAll that
 * lets POST /agents/{id}/invoke take an organisationId in the body; nothing
 * else, see lib/auth/permissions.js) and an AuthKey, then prints the bearer
 * token for polite-ai's LLM_AGENT_ANALYSIS_TOKEN.
 *
...
async function main() {
  ...
  // Upsert the synthetic user by email; role=analysisService, status=active, no organisation.
  const found = await client.query('SELECT id FROM users WHERE email = $1', [EMAIL]);
  let userId;
  if (found.rows.length) {
    userId = found.rows[0].id;
    await client.query(`UPDATE users SET role = 'analysisService', status = 'active', organisation_id = NULL, updated_at = now() WHERE id = $1`, [userId]);
    console.log(`reusing analysisService user ${userId} (${EMAIL})`);
  } else {
    userId = randomUUID();
    await client.query(
      `INSERT INTO users (id, name, email, email_verified, phone, phone_verified, picture, role, status, created_at, updated_at)
       VALUES ($1, 'Call Analysis Service', $2, true, '', false, '', 'analysisService', 'active', now(), now())`,
      [userId, EMAIL],
    );
    console.log(`created analysisService user ${userId} (${EMAIL})`);
  }

  // Upsert the AuthKey. role_restriction='analysisService' pins the key to that
  // vocabulary even if the user's role ever changes. Stored as the role NAME,
  // never as a literal statement map, so a later change to the role reaches
  // this key on the next deploy with no re-mint.
  await client.query(
    ...
    [KEY, userId, JSON.stringify('analysisService'), new Date('2099-01-01T00:00:00Z')],
  );
