/**
 * Save-time rules for the xAI Grok voice rows (docs/grok.md).
 *
 * Two things the Grok Voice Agent API accepts at session time and fails on
 * later, so the agent is rejected when it is saved instead:
 *
 * - a function tool whose name collides with one of xAI's server-side tools
 *   (the session fails its first response with a server error when that
 *   server tool is enabled; the platform never enables one, so this is
 *   defensive);
 * - server-side tools smuggled in through `vendorSpecific.xai.session`: a
 *   `tools` array, or an entry of type `mcp`, `web_search`, `x_search` or
 *   `file_search`. They run outside the platform's tool logging, RBAC and
 *   outbound filters and are not offered (plan decision 14).
 *
 * The workers strip `tools` from the merged block as a second line.
 */
