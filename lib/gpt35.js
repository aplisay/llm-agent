...
class Llm {
  ...
  constructor(logger, user, prompt) {
    this.prompt = prompt || initialPrompt;
    ...
    logger.info({thisPrompt: this.prompt, prompt}, 'NEW GPT35agent');
  }
  ...
  get voiceHints() {
    let hints = this._hints || [...new Set(this.prompt.split(/[^a-zA-Z0-9]/))].filter(h => h.length > 2);
