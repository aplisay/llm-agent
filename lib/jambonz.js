...
const getServiceProvider = async () => {
  console.log({ api }, 'getting serviceprovider');
  let { data } = await api.get("ServiceProviders");
  console.log({ data }, 'got serviceprovider');
  let serviceProviderSid = data.length && data[0]?.service_provider_sid;
  let accountSid;
  if (serviceProviderSid) {
    console.log({ serviceProviderSid, data }, 'got serviceprovider');
    ...
    console.log({ accountSid, data }, 'got account');
  }
  else {
    numberApi = api;
  }
