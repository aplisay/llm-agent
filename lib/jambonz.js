...
class Jambonz {
  ...
  async updateNumber(sid, { number, carrier, application }) {
    if (!sid)
      throw new Error('No SID in pudate call');
    let { data } = await api.put(`PhoneNumbers/${sid}`, {
      voip_carrier_sid: carrier, application_sid: application
    });
