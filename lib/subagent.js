...
import { functionList, resultFunctions } from './decision-questions.js';
...
export async function runSubagent({
  ...
  const functions = functionList(agent.functions);
  const resultNames = resultFunctions(functions).map((f) => f.name);

  // A decision-kind driver (docs/typesafe-jev.md) answers its result schema in
  // one exchange: no chat harness, no tool loop, so the prompt goes in bare.
  const decision = Implementation.kind === 'decision';
  const llm = new Implementation({
    logger,
    user: `subagent-${agent.id}`,
    prompt: decision ? (agent.prompt || '') : buildSystemPrompt(agent, resultNames),
  ...
  try {
    if (decision) {
      // The full functions (llmFunctions() strips x-levels and x-descriptions) and the untouched input.
      const { result, transcript: decided, usage: decidedUsage } = await llm.decide({ functions, input });
      addUsage(decidedUsage);
      return { result, complete: true, transcript: decided, usage: usage() };
    }
    ...
    // A decision driver reports a schema it cannot answer (400) or a failed
    // exchange (502) with a status: surface it as a SubagentError so the routes
    // answer with that status instead of a generic 500.
    const err = decision && !(e instanceof SubagentError) && Number.isInteger(e?.status)
      ? Object.assign(new SubagentError(e.message, e.status), e.requestId !== undefined ? { requestId: e.requestId } : {})
      : e;
...
/**
 * Does the agent's organisation allow its model? The organisation's own list
 * is the gate (no member principal is present on an autonomous or
 * on-behalf path); an unrestricted organisation, or an agent with none,
 * allows everything. Shared by runSubagentById and the invoke route's
 * on-behalf path, so both apply one rule.
 */
export async function organisationAllowsModel(agent) {
  ...
  const { ORGANISATION_RBAC_ATTRIBUTES } = await import('./auth/permissions.js');
