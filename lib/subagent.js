...
export async function runSubagent({
  ...
  // one exchange: no chat harness, no tool loop. It reads the agent prompt and
  // the raw input itself, so the harness prompt would only be sent as state.
  ...
  const llm = new Implementation({
    ...
    // The full result schema and the untouched input, for decision drivers.
    // llmFunctions() above strips every keyword but type, description and enum,
    // which loses x-levels and x-descriptions. Other drivers ignore these.
  ...
  if (decision) {
    try {
      const { result, transcript: decisionTranscript, usage: decisionUsage } = await llm.decide();
      addUsage(decisionUsage);
      return { result, complete: true, transcript: decisionTranscript, usage: usage() };
    } catch (e) {
      // Driver errors carry a status (400 for a schema the model cannot answer,
      // 502 for a failed exchange); surface them as SubagentError so /invoke
      // and /agent-db/subagent answer with it instead of a generic 500.
      const err = e instanceof SubagentError || !Number.isInteger(e?.status)
        ? e
        : Object.assign(new SubagentError(e.message, e.status), {
