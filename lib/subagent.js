 ...
 * voice workers).
 */
function llmFunctions(functions = []) {
...
export async function runSubagent({
  ...
  for (let i = 0; i < maxTurns; i++) {
    ...
    if (calls && calls.length) {
      ...
      if (resultCall) {
        return { result: resultCall.input ?? {}, complete: true, transcript };
      }
    ...
    if (!resultNames.length && lastText) {
      return { result: { text: lastText }, complete: true, transcript };
    }
    if (i === maxTurns - 1) {
      break;
    }
  ...
  return { result: { text: lastText ?? null }, complete: false, transcript };
}
