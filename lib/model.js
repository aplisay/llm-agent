...
class Model {
   ...
   * 
   * @param {Object} params Model creation parameters
   * @param {string} params.modelName supported LLM agent name, must be one of #Model.agents
  ...
  constructor({ wsServer, logger }) {
    logger.info({ wsServer }, 'Model create wsServer WS with voices');
  ...
  static async listModels() {
    let agentImplementations = Object.entries(Model.agents)
  ...
  async activate({ number, options = {} } = {}) {
    ...
    let type = Model.models[model].handler;
    let llm = new Model.models[model]({ logger, ...agent.dataValues });
    try {
      ...
      return { ...activation, number: allocated, socket: progressPath };
    }
    catch (e) { 
   ...
   */
  async create() {
    let llmClass = Model.agents[this.implementationName].implementation;
  ...
  async destroy() {
    try {
      ...
      delete this.number;
      this.application && this.application.application_sid && await this.jambonz.deleteModel(this.application.application_sid);
      delete this.application;
    }
    catch (error) {
      this.logger.error({ error, jambonzModel: this.application }, 'Model destroy');
    }
    finally {
      Model.live = Model.live?.filter(a => a.name !== this.name);
    }
  }
