...
class XiLabs {
   ...
   */
  async listVoices(languageCode = 'en') {
    return (await this.list).filter(voice => voice?.language?.startsWith(languageCode))
      .map(v => (console.log({ v, language: v.language }, 'VOICE'), v))
      .reduce((o, v) => (console.log({ o, v }), ({ ...o, [v.language]: [...(o[v.language] || []), v] })), {});
  }
