...
class XiLabs {
      ...
      fetch(URI, options)
       ...
       .then(response => 
         resolve(response?.voices?.map(voice => {
           ...
           logger.info({ name: voice.name, accent, language, decorator }, 'xilabs voice')
           return { name: voice.name, gender, description: `${d1} - ${decorator&&(decorator+" ")}${age} ${d2}`, language: language || accentMap.default };
          }))
        )
  ...
  async listVoices(languageCode) {
    ...
    logger.info({ list: await list }, 'xilabs listVoices');
