...
export function loadRegclientConfig(env = process.env) {
  ...
  return {
    ...
    // Accept the previous token for verification during rotation so outstanding probe handles remain valid; never sign
    // with it. See PR #265.
...
/**
 * Transport failures mean unknown capability and must not be cached; only certificate rejection proves an
 * incompatible endpoint here. See PR #265.
 */
...
/**
 * Return 501 for known unsupported nodes; reserve 504 for nodes that failed to answer. See PR #265.
 */
