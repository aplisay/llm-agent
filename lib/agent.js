...
const service = ({ name, llmClass, logger, wsServer, makeService, prompt, handleClose = () => (null)}) => {
  let path = `/agent/${name}`;
  ...
  socket.on('session:new', async (session) => {
    ...
    session.locals = {
      ...
      agent: new llmClass(logger, session.call_sid, usePrompt),
      progress
    };
  ...
  return { changePrompt, socketPath: progressPath };
};
...
const sendCompletion = async (session, evt) => {
  ...
  session.locals.progress.send(JSON.stringify({ completion: text }));
  if (hangup) {
    session
      .say({ text })
      .hangup()
