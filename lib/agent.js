...
class JambonzSession {
  ...
  async handler() {
    ...
    try {
      this.watchdog(() => {
        ...
        session.say({ text: speak(text), ...this.sayOptions }).send();
        return () => {
          let text = `I'm sorry, I haven't heard anything from you or the agent for a while so hanging up now`
          session.say({ text: speak(text), ...this.sayOptions }).hangup().send();
          progress && progress.send({ inject: text });
        };
      })
  ...
  async watchdog(watchdogFcn, watchdogTimeout = 20000) {
    ...
    this.watchdogFcn && (this.watchdogTimer = setTimeout(() => {
  ...
  async #sendCompletion({ text, hangup, data, calls }) {
    ...
    while (calls && calls.length) {
      ...
      text && (waitingId = uuid()) && session
        .say({ text: speak(text), ...this.sayOptions, id: waitingId })
        .send();
      this.watchdog();
      logger.debug({ text: speak(text), ...this.sayOptions, id: waitingId }, 'sent text');
      ...
      logger.debug({ waitingId}, 'waiting for say completion');
      await this.#waitFor(waitingId);
      logger.debug({ waitingId }, 'got say completion');
    }
