...
class JambonzSession {
  ...
  async handler() {
    ...
    try {
      session.listen({
        ...
        .send();
      let { text: completion } = await agent.initial();
      logger.info({ completion }, 'Got initial completion');
      progress && progress.send({ completion });
  ...
  async #sendCompletion(evt) {
    ...
    calls && progress && progress.send({ apiCalls: calls });
...
class Agent {
  ...
  constructor({ implementationName, model, name, llmClass, logger, wsServer, makeService, prompt, options, callbackUrl, handleClose = () => (null) }) {
    let path = `/agent/${name}`;
    let progressPath = `/progress/${name}`;
    let socket = makeService({ path });
    this.progress = { send: () => (null) };

    Object.assign(this, { name, path, socketPath: progressPath, wsServer, socket, llmClass, logger: logger.child({ name, implementationName }), prompt, handleClose, callbackUrl });
    ...
    socket.on('session:new', async (session) => {
      ...
      let s = new JambonzSession({
        ...this,
        session,
        agent: new llmClass({ logger, user: session.call_sid, model, prompt: this.prompt, options: this.options }),
