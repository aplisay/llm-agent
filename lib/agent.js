...
class Agent {
  ...
  constructor({ name, llmClass, logger, wsServer, makeService, prompt, options, handleClose = () => (null) }) {
    ...
    Object.assign(this, { name, path, socketPath: progressPath, wsServer, socket, llmClass, logger, prompt, handleClose });
    ...
    wsServer.createEndpoint(progressPath, (ws) => {
      ...
      logger.info({ ws, name: this.name, progressPath: this.socketPath }, 'got connection on ws');
      this.progress = {
        send: async (msg) => {
          logger.info({ msg, progressPath: this.socketPath }, `sending WS message: ${msg}`);
          ws.send(msg);
        }
      };
