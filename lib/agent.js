...
class Agent {
  ...
  constructor({ modelName, name, llmClass, logger, wsServer, makeService, prompt, options, handleClose = () => (null) }) {
    let path = `/agent/${name}`;
    let progressPath = `/progress/${name}`;
    let socket = makeService({ path });
    this.progress = { send: () => (null) };

    Object.assign(this, { name, path, socketPath: progressPath, wsServer, socket, llmClass, logger: logger.child({ name, modelName }), prompt, handleClose });
    ...
    wsServer.createEndpoint(progressPath, (ws) => {
      ...
      this.progress = { send: async (msg) => ws.send(JSON.stringify(msg)) };
