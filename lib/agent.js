...
class Agent {
  ...
  constructor({ name, llmClass, logger, wsServer, makeService, prompt, options, handleClose = () => (null) }){
    ...
    socket.on('session:new', async (session) => {
      ...
      session
        .on('/prompt', this.onUserPrompt.bind(null, session))
    ...
    Object.assign (this, { changePrompt, changeOptions, socketPath: progressPath })
  }

  async onUserPrompt (session, evt){
    const { logger, self } = session.locals;
    logger.info(`got speech evt: ${JSON.stringify(evt)}`);

    switch (evt.reason) {
      case 'speechDetected':
        self.sendCompletion(session, evt);
        break;
      case 'timeout':
        self.goodbye(session);
        break;
      default:
        self.reply();
        break;
  ...
  async sendCompletion (session, evt) {
    ...
    hangup && session.locals.progress.send(JSON.stringify({ hangup }));
  ...
  async onClose (session, code, reason) {
    const { logger, progress } = session.locals;
    progress.send(JSON.stringify({ hangup: true }));
  ...
  async destroy() {
    this.sessions.forEach(session => (session?.locals?.ws && session?.locals?.ws?.close()));
 }
}
