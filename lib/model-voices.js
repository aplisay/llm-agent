...
import { isLivekitPipelineModelId, livekitModelIdFlags } from './models/livekit.js';
import { isPipecatGptLiveModelId, isPipecatPipelineModelId, pipecatModelIdFlags } from './models/pipecat.js';
import { openaiLiveVoiceTree } from './voices/openai-live.js';
...
function livekitRealtimeVoiceVendorsForRestModelId(rest) {
  ...
  if (provider === 'xai') return new Set(['xAI']);
 ...
 *
 * @param {string} rest e.g. `openai/gpt-realtime`, `google/gemini-2.5-flash-native-audio-preview-12-2025`
 * @returns {Set<string> | null}
...
/**
 * The model's own voice tree for a Pipecat realtime row: the handler's merged
 * catalogue scoped to the row's provider, with the `OpenAI` block swapped for
 * the GPT-Live list on a GPT-Live row (lib/voices/openai-live.js). GPT-Live
 * shares the `openai` provider segment with OpenAI Realtime but takes a
 * different voice list, and the API rejects a Realtime-only name.
 *
 * @param {string} rest e.g. `openai/gpt-live-1`
 * @param {Record<string, Record<string, unknown[]>>} fullTree the handler's `voices`
 * @returns {Record<string, Record<string, unknown[]>>}
 */
function pipecatRealtimeVoiceTreeFor(rest, fullTree) {
  const scoped = filterVoiceTreeVendors(fullTree, pipecatRealtimeVoiceVendorsForRestModelId(rest));
  if (isPipecatGptLiveModelId(rest)) {
    return { ...scoped, ...openaiLiveVoiceTree() };
  }
  return scoped;
}
...
/**
 * Realtime rows that may pair the model with an external TTS (text-output mode):
 * the `externalTts` row flag from the handler's model registry, surfaced on
 * GET /models as `hasExternalTts`. Pipeline rows never carry it.
 *
 * @param {string} modelName
 * @returns {boolean}
 */
export function modelSupportsExternalTts(modelName) {
  const { handler, rest } = splitHandlerModel(modelName);
  if (!handler || !rest) return false;
  if (handler === 'pipecat') return pipecatModelIdFlags[rest]?.externalTts === true;
  if (handler === 'livekit') return livekitModelIdFlags[rest]?.externalTts === true;
  return false;
}

/** Explicit prototype opt-in; this does not advertise a native text modality. */
export function gptLiveTranscriptTtsEnabled({ modelName, options }) {
  const { handler, rest } = splitHandlerModel(modelName);
  return handler === 'pipecat'
    && isPipecatGptLiveModelId(rest)
    && options?.tts?.experimentalTranscript === true
    && isExternalTtsForModel({ modelName, vendor: options?.tts?.vendor });
}

/**
 * Rows that delegate reasoning and tool use to a backend text agent named by a
 * builtin `delegate` function (docs/gpt-live.md): the `delegation` row flag from
 * the handler's model registry, surfaced on GET /models as `hasDelegation`.
 *
 * @param {string} modelName
 * @returns {boolean}
 */
export function modelSupportsDelegation(modelName) {
  const { handler, rest } = splitHandlerModel(modelName);
  if (!handler || !rest) return false;
  if (handler === 'pipecat') return pipecatModelIdFlags[rest]?.delegation === true;
  if (handler === 'livekit') return livekitModelIdFlags[rest]?.delegation === true;
  return false;
}

/**
 * The `options.tts.vendor` value that means "the model's own voice" on a realtime
 * model: its provider segment (`ultravox`, `openai`, `google`). `google` is both a
 * realtime provider and a TTS vendor, so it is native on a Gemini row and external
 * anywhere else. Same table as REALTIME_NATIVE_TTS_VENDORS in
 * agents/pipecat/pipecat_aplisay/realtime_tts.py.
 *
 * @param {string} modelName
 * @returns {string} lowercased provider segment, or '' when there is none
 */
export function nativeTtsVendorForModel(modelName) {
  const { rest } = splitHandlerModel(modelName);
  const s = String(rest || '');
  const i = s.indexOf('/');
...
/**
 * True when `vendor` asks a realtime LiveKit/Pipecat model for an external TTS:
 * set, and not the model's own provider (any `/model` scoping ignored). False on
 * pipeline rows, whose TTS is always a discrete stage, and on other handlers.
 * Whether the row can honour the request is {@link modelSupportsExternalTts}.
 *
 * @param {{ modelName: string, vendor?: string | null }} p
 * @returns {boolean}
 */
export function isExternalTtsForModel({ modelName, vendor }) {
  const { handler, rest } = splitHandlerModel(modelName);
  if (!handler || !rest) return false;
  if (handler !== 'livekit' && handler !== 'pipecat') return false;
  if (handler === 'livekit' && isLivekitPipelineModelId(rest)) return false;
  if (handler === 'pipecat' && isPipecatPipelineModelId(rest)) return false;
  const v = String(vendor || '').trim().split('/')[0].trim().toLowerCase();
  if (!v) return false;
  return v !== nativeTtsVendorForModel(modelName);
}

/**
 * The discrete-TTS catalogue a handler's worker can render: the vendor tree its
 * pipeline models use. Also offered on realtime rows flagged `externalTts`, where
 * a vendor other than the model's own switches the model to text output and this
 * TTS speaks (docs/realtime-external-tts.md).
 *
 * @param {string} handler
 * @param {import('./voices/index.js').default} voicesInstance
 * @returns {Promise<Record<string, Record<string, unknown[]>>>}
 */
async function discreteTtsTreeFor(handler, voicesInstance) {
  const raw = await pipelineVendorTrees(voicesInstance);
  if (handler === 'livekit') {
    return transformVoiceTreeForLiveKitPipeline(voiceTreeWithoutGoogleVendorForPipelineList(raw));
  }
  if (handler === 'pipecat') return pipecatPipelineTtsTree(raw);
  return {};
}
 ...
 */
const PIPELINE_TTS_VENDORS = new Set(['elevenlabs', 'cartesia', 'deepgram', 'google', 'neuphonic']);

/**
 * TTS vendors the pipecat worker itself can build (`build_tts_service` in
 * agents/pipecat/pipecat_aplisay/voice_session.py). A discrete-TTS request on a
 * pipecat realtime row (external TTS, fallback message) is rendered by that
 * worker, so only these count there; `google` is a LiveKit-only TTS vendor.
 */
const PIPECAT_WORKER_TTS_VENDORS = new Set(['cartesia', 'elevenlabs', 'deepgram', 'neuphonic']);
...
export async function getTtsVendorsForAgentValidation({ modelName, Handler, voicesInstance, discreteTts = false }) {
  ...
  if (handler === 'pipecat' && (discreteTts || isPipecatPipelineModelId(rest))) {
    return new Set([
      ...
      ...(discreteTts ? PIPECAT_WORKER_TTS_VENDORS : PIPELINE_TTS_VENDORS),
    ]);
  }
  if (handler === 'pipecat') {
    return collectVendorNamesFromTree(pipecatRealtimeVoiceTreeFor(rest, await Handler.voices));
  }
 ...
 * by a discrete TTS — `options.fallback.message`, which plays when the model could
 * not be started at all, and `options.tts` itself when the agent pairs a realtime
 * model with an external TTS ({@link isExternalTtsForModel}) — must be validated
 * against that catalogue instead. Validating it against the model's own voices
 * would reject exactly the configuration that works (an ElevenLabs voice announcing
 * an Ultravox outage) and accept ones that cannot be rendered.
 *
 * @param {object} p
 * @param {string} p.modelName
...
export async function getVoiceNamesForAgentValidation({ modelName, Handler, voicesInstance, discreteTts = false }) {
  ...
  if (handler === 'pipecat') {
    return collectVoiceNamesFromTree(pipecatRealtimeVoiceTreeFor(rest, await Handler.voices));
  }
...
/**
 * Match Unicode word starts, allowing prefixes without matching male inside female; JavaScript \b is ASCII-only. See
 * PR #157.
 *
...
export async function getModelVoiceLocales({ modelName, voicesInstance }) {
  ...
  if (handler === 'livekit') {
    ...
    voiceTree = pipecatRealtimeVoiceTreeFor(rest, voiceTree);
  }
  if (modelSupportsExternalTts(modelName)) {
    // The model's own voices win on a shared vendor key: that vendor is native there.
    voiceTree = { ...(await discreteTtsTreeFor(handler, voicesInstance)), ...voiceTree };
  }
  const stack =
...
async function buildModelVoiceTree({ modelName, voicesInstance }) {
  ...
  if (handler === 'livekit') {
    ...
    tree = pipecatRealtimeVoiceTreeFor(rest, tree);
  }
  if (modelSupportsExternalTts(modelName)) {
    // A realtime row that can run with an external TTS also offers the discrete
    // catalogue; picking one of those vendors is what switches the model to text
    // output. The model's own voices win on a shared vendor key (native there).
    tree = { ...(await discreteTtsTreeFor(handler, voicesInstance)), ...tree };
  }
