...
export const statements = {
  ...
  // `credit` = top up the org's balance (the Stripe-webhook seam; held by the
  // least-privilege `billingService` role, NOT bundled into setRate).
  organisation: ['create', 'read', 'readAll', 'update', 'delete', 'verify', 'setLimits', 'setRate', 'credit', 'setPermissions'],
...
export const roles = {
  ...
  // synthetic service user holds an AuthKey with this role; it can ONLY credit an
  // org's balance (no read, no rate assignment, no product access) — least
  // privilege for a server-to-server credential that lives in another process.
  billingService: {
    statements: {
      organisation: ['credit'],
    },
