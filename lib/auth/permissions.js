...
export const statements = {
  ...
  // `readAll` is the cross-tenant marker for the analysisService seam: it lets
  // POST /agents/{id}/invoke name an organisation in the body. No role for a
  // person holds it.
  agent: ['create', 'read', 'update', 'delete', 'deploy', 'listen', 'originate', 'invoke', 'readAll'],
...
export const roles = {
  ...
  superAdmin: {
    statements: {
      ...OWNER_STATEMENTS,
      // The superset rule: superAdmin holds agent:readAll so it can grant the
      // analysisService role. The invoke route still refuses `organisationId`
      // from any principal that has an organisation of its own.
      agent: [...OWNER_STATEMENTS.agent, 'readAll'],
      call: ['read', 'listen', 'prune'],
      usage: ['read', 'readAll'],
      rate: ['read', 'readAll', 'create', 'update', 'delete'],
      tariff: ['read', 'readAll', 'create', 'update', 'delete'],
  ...
  // Machine identity for the polite-ai post-call analysis seam
  // (docs/typesafe-jev.md, docs/call-hooks.md). The receiver of a call hook
  // invokes the organisation's analysis agent with no user session, so it
  // holds `agent:invoke` and the cross-tenant `agent:readAll` that lets
  // POST /agents/{id}/invoke take an `organisationId` in the body, and nothing
  // else: no agent read, create or update, no calls, no usage, no billing.
  // Keys minted by scripts/provision-analysis-service.mjs store role_restriction
  // as the ROLE NAME, so a later change here reaches them with no re-mint.
  analysisService: {
    statements: {
      agent: ['invoke', 'readAll'],
    },
  },
};
