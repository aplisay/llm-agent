...
export const statements = {
  ...
  // `readAll` lets POST /agents/{id}/invoke name an organisation in the body; only the
  // analysisService machine role holds it, and no agent read or list route widens on it.
...
export const roles = {
  ...
  // Machine identity for the polite-ai post-call analysis seam (docs/call-hooks.md):
  // invoke an organisation's decision-model agent with no user session. The
  // model list confines it to decision models, so a leaked key cannot drive a
  // generative agent's tools or the built-in chat agents. Minted by
  // scripts/provision-analysis-service.mjs with role_restriction as the role name.
  analysisService: {
    statements: {
      agent: ['invoke', 'readAll'],
    },
    models: ['text:typesafe/'],
  },
};
