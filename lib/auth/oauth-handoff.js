...
<noscript><button type="submit">Continue to polite.ai</button></noscript>
  ...
  if (limiter) server.get('/api/oauth-handoff', limiter, handler);
  else server.get('/api/oauth-handoff', handler);
  logger.info({ destination: action }, 'mounted oauth-handoff at /api/oauth-handoff');
}
