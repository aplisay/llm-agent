...
const schemaVersion = 44;  // 44: billing — RateCard table + organisations.{rate_history,balance,billing_config,billing_blocked} + usage_records billing/cost columns
...
class Call extends Model {
  ...
  async end() {
    const call = this;
    try {
      // The worker fires POST /start and POST /end as two SEPARATE requests, each
      // doing its own findByPk -> mutate -> save on a distinct instance with no
      // shared transaction or row lock. Under load (or when a setup-timeout
      // abandons an in-flight /start) /end can read the row before /start has
      // committed startedAt. If startedAt is missing on this instance, re-read it
      // (it may have been committed since our findByPk) before computing duration —
      // this closes the read-before-write window without clobbering a startedAt
      // already set on the instance.
      if (!call.startedAt) {
        const fresh = await Call.findByPk(call.id, {
          attributes: ['id', 'startedAt', 'createdAt'],
        }).catch(() => null);
        if (fresh?.startedAt) call.startedAt = fresh.startedAt;
        if (!call.createdAt && fresh?.createdAt) call.createdAt = fresh.createdAt;
      }
      call.endedAt = new Date();
      // Defensive duration: fall back to createdAt (always set) when startedAt is
      // still absent, so an ended call ALWAYS gets a numeric duration — and hence
      // a voice/milliseconds usage row. The old `startedAt && (...)` returned the
      // falsy startedAt, persisting duration=NULL and leaving the call unbilled.
      const startInstant = call.startedAt || call.createdAt || call.endedAt;
      call.duration = call.endedAt.valueOf() - new Date(startInstant).valueOf();
      call.live = false;
  ...
  async recordUsageMinutes() {
    ...
    try {
      ...
      const media = Call.mediaFromIds(call.callerId, call.calledId);
      ...
      if (!created) {
        await row.update({ quantity: call.duration, finalised: true, media });
      }
    } catch (e) {
