...
const schemaVersion = 49;  // 49: billing Phase D3 — `calls.outbound_trunk_id` (the real Trunk.id a carried
...
class Call extends Model {
  ...
  async recordUsageMinutes() {
    ...
    try {
      ...
      // Destination (carrier) billing gate (Phase D3): a CARRIED outbound leg on a
      // trunk the org does NOT own → freeze the dialled number as the anchor the
      // costing resolver longest-prefix-matches against the org's tariff. An
      // org-owned trunk, or no outbound trunk (inbound / WebRTC / REFER /
      // registration egress), is not destination-billable. The resolver normalises
      // this raw number using the resolved tariff's own default country.
      let destinationRaw = null;
      if (call.outboundTrunkId && call.calledId && call.organisationId) {
        const owned = await TrunkOrganisation.findOne({
          where: { trunkId: call.outboundTrunkId, organisationId: call.organisationId },
          attributes: ['trunkId'],
        });
        if (!owned) destinationRaw = call.calledId;
      }
