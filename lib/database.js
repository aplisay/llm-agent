...
class Call extends Model {
  ...
  async start() {
    const call = this;
    await agentConcurrencyLimits.reserveForCall(call);
    try {
...
class InstanceConcurrency extends Model {}

InstanceConcurrency.init(
  {
    callId: {
      type: DataTypes.UUID,
      primaryKey: true,
  ...
  {
    sequelize,
    modelName: 'InstanceConcurrency',
    tableName: 'instance_concurrency',
    timestamps: true,
    underscored: true,
    charset: 'utf8',
    collate: 'utf8_general_ci',
...
class UserConcurrency extends Model {}

UserConcurrency.init(
  {
    callId: {
      type: DataTypes.UUID,
      primaryKey: true,
  ...
  {
    sequelize,
    modelName: 'UserConcurrency',
    tableName: 'user_concurrency',
    timestamps: true,
    underscored: true,
    charset: 'utf8',
    collate: 'utf8_general_ci',
...
class OrganisationConcurrency extends Model {}

OrganisationConcurrency.init(
  {
    callId: {
      type: DataTypes.UUID,
      primaryKey: true,
  ...
  {
    sequelize,
    modelName: 'OrganisationConcurrency',
    tableName: 'organisation_concurrency',
    timestamps: true,
    underscored: true,
    charset: 'utf8',
    collate: 'utf8_general_ci',
...
InvocationLog.belongsTo(Agent, { foreignKey: 'agentId', onDelete: 'SET NULL' });
InvocationLog.belongsTo(Instance, { foreignKey: 'instanceId', onDelete: 'SET NULL' });
...
const databaseStarted = sequelize.authenticate()
  ...
  .then(() => (doUpgrade && Metadata.sync({ alter: true })
    ...
    .then(() => InstanceConcurrency.sync({ alter: true }))
    .then(() => UserConcurrency.sync({ alter: true }))
    .then(() => OrganisationConcurrency.sync({ alter: true }))
