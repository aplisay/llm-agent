...
// 37: Agent.mcpServers (remote MCP servers exposed to the model as tools)
// 38: agent_sets table; Agent.type / label / agentSetId
// 39: usage_records table (token/character/minute usage ledger)
// 40: users.signup_method ('firebase' | 'better-auth') for the parallel-auth phase
    ...
    try {
      ...
      const [row, created] = await UsageRecord.findOrCreate({
        ...
        defaults: {
          ...
          technology: 'voice', provider, detail, unit: 'milliseconds',
          quantity: call.duration, finalised: true,
        },
      });
      if (!created) {
        await row.update({ quantity: call.duration, finalised: true });
      }
    } catch (e) {
  ...
  static async import(user) {
    if (User.cachedUsers.has(user.id)) {
      ...
      try {
        ...
        let [updatedUser,] = await User.upsert({ id, name, email, emailVerified, phone, phoneVerified, picture, role, signupMethod: 'firebase' });
        return updatedUser;
      } catch (e) {
