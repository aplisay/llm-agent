    ...
    validate: {
      async handlerLimitations() {
        ...
        if (this.options?.tts?.voice || this.options?.tts?.vendor) {
          ...
          // A realtime model asked for a TTS vendor other than its own provider: the
          //  model runs in text-output mode and that TTS speaks its text
          //  (docs/realtime-external-tts.md). Only rows flagged externalTts can do it;
          //  on any other realtime model the vendor is rejected here, with the fix in
          //  the message, rather than silently ignored at call time. When it applies,
          //  voice and vendor are validated against the discrete TTS catalogue, not
          //  the model's own voices, exactly as options.fallback.message is below.
