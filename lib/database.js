...
import { validateXaiVoiceAgent } from './grok-limits.js';
import { validateDecisionAgent } from './decision-limits.js';
...
const schemaVersion = 66;  // 66: registrar accounts (regserver) — `phone_registrations.mode` ('client' |
                           ...
                           //     per call, a floor on connect + per-minute × duration).
                           // 51: billing Phase D5 — tariff peak/off-peak + rounding + call-start: tariffs gains
                           //     timezone, schedule (JSONB per-weekday peak window), call_start_micros,
                           //     rounding_seconds; tariff_prefixes replaces per_minute_micros with
                           //     peak_per_minute_micros + off_peak_per_minute_micros.
                           // 50: billing Phase D3 — `trunks.chargeable` (positive per-trunk flag: our public/
...
// Resolve cache misses from Call -> Instance.streamLog: log writes may reach a different process from call creation.
// Cache both enabled and disabled results with a bounded size; see PR #293.
    ...
    validate: {
      async handlerLimitations() {
        ...
        if (this.options?.tts?.voice || this.options?.tts?.vendor) {
          ...
          // Validate external voices against the discrete TTS catalogue only on supported rows or the GPT-Live opt-in.
          // See docs/realtime-external-tts.md.
          const externalTts = isExternalTtsForModel({
            modelName: this.modelName,
            vendor: this.options?.tts?.vendor,
          });
          if (externalTts && !modelSupportsExternalTts(this.modelName)
            && !gptLiveTranscriptTtsEnabled(this)) {
            throw new Error(
              `TTS vendor "${this.options.tts.vendor}" not supported by ${this.modelName}: `
              + 'this realtime model speaks with its own voice (leave options.tts.vendor unset '
              + `or set it to "${nativeTtsVendorForModel(this.modelName)}")`);
          }
        ...
        if (this.options?.fallback?.message !== undefined && this.options?.fallback?.message !== null) {
          // Validate fallback voices before call setup so configuration errors cannot disable announcements during failure. See
          // docs/agent-failover.md.
          ...
          // Fallback announcements always use a discrete TTS, including for realtime agents; validate against that catalogue.
          // See lib/fallback-message/CONTRACT.md.
        ...
        if (this.options?.bridgedTransferToAgent !== undefined && this.options?.bridgedTransferToAgent !== null) {
          ...
          validateBridgedTransferToAgentShape(this.options.bridgedTransferToAgent, {
        ...
        // Grok voice rows: xAI's reserved function names and server-side tools
        //  in vendorSpecific fail at call time, so they are refused here (docs/grok.md).
        validateXaiVoiceAgent({ modelName: this.modelName, functions: this.functions, options: this.options });
        // Decision-kind rows (docs/typesafe-jev.md): pinned id, one answerable
        //  `result` function, no tool loop, no voice options; `options.decision`
        //  is accepted on these rows only.
        validateDecisionAgent({ modelName: this.modelName, functions: this.functions, options: this.options, mcpServers: this.mcpServers });
        const functions = Object.entries(this.functions || {});
        // A GPT-Live agent names its backend text agent with ONE builtin `delegate`
        //  function (docs/gpt-live.md): the session has a single backend, so a
        //  second declaration can only be a mistake.
        const delegates = functions.filter(([, f]) => f?.implementation === 'builtin' && f?.platform === 'delegate');
        if (delegates.length > 1) {
          throw new Error(
            `An agent may declare at most one delegate function (found ${delegates.map(([n, f]) => f.name || n).join(', ')})`);
        }
        return functions.every(([name, func]) => {
          ...
          switch (func.platform) {
            ...
            case 'subagent':
              ...
              break;
            case 'delegate':
              // Names the backend text agent of a two-layer voice model (GPT-Live):
              //  only rows flagged `delegation` in the model registry run one.
              if (!modelSupportsDelegation(this.modelName)) {
                throw new Error(
                  `Model ${this.modelName} does not delegate to a backend agent, but a delegate function was found in functions `
                  + '(delegate is only valid on models flagged hasDelegation in GET /models, such as pipecat:openai/gpt-live-1)');
              }
              break;
            case 'transfer_summary':
              // Collects the result of the summaryAgent pre-fired at a
              // bridgedTransferToAgent hand-back (docs/transfer-back-plan.md).
              // Rides on the same capability set as the hand-back itself.
              if (!Handler.hasAgentTransfer || !Handler.hasSubagent) {
                throw new Error(`Model ${this.modelName} does not support hand-back summaries, but a transfer_summary function was found in functions`);
              }
              break;
            case 'send_dtmf':
              // Out-of-band (RFC 4733) DTMF over a SIP call. Only the LiveKit
              // and Pipecat workers can synthesise telephone-event tones; on a
              // WebRTC/browser session the builtin errors at call time.
              if (!Handler.hasDtmf) {
                throw new Error(`Model ${this.modelName} does not support sending DTMF, but a send_dtmf function was found in functions`);
              }
              break;
            case 'result':
              if (agentType !== 'text') {
              ...
              break;
            case 'create_agent_set':
            case 'update_agent_set':
            case 'patch_agent_set':
            case 'list_voices':
            case 'ask_user':
            case 'test_agent':
            case 'request_review':
          ...
          switch (func.platform) {
            ...
            case 'subagent':
              ...
              return true;
            case 'delegate': {
              // Same target rules as subagent (static UUID / label, or metadata;
              //  never generated). The voice model never calls it, so it takes no
              //  other parameters: the backend's inputs are the conversation.
              validateAgentTarget(name, func);
              const extra = Object.keys(func.input_schema?.properties || {}).filter((key) => key !== 'agent');
              if (extra.length) {
                throw new Error(
                  `${name}: delegate function takes only the "agent" parameter (found ${extra.join(', ')})`);
              }
              return true;
            }
            case 'send_dtmf': {
              const digits = func.input_schema?.properties?.digits;
              if (!digits) {
                throw new Error(`Send_dtmf function must include a parameter called "digits" (function ${name})`);
              }
              // Unlike transfer's `number`, the digit string may be `generated`
              // (the LLM enters what it learned in-call, e.g. an IVR selection or
              // account number): sending tones on the existing call is not a
              // redirect, so there is no anti-fraud constraint on the source.
              // When the value is fixed by the agent designer (static), validate
              // the alphabet up-front; generated/metadata values are checked at
              // call time by the worker.
              if (digits.source === 'static' && digits.from !== undefined
                && !/^[0-9*#]+$/.test(String(digits.from))) {
                throw new Error(`Send_dtmf static "digits" may only contain the characters 0-9, * and # (function ${name})`);
              }
              return true;
            }
            case 'result':
...
Instance.init({
  ...
  // Listener-level transfer overrides (docs/transfer-back-plan.md): when set,
  //  each field wholesale-replaces the same-named agent option for every call
  //  on this listener. `label:` references are resolved to UUIDs (with
  //  fromLabel annotations) at activation time, so the stored map is always
  //  UUID-valued like the agent-side option.
...
class Call extends Model {
  /**
   * Keep numbering and INSERT in one transaction; managed transactions roll back hook failures and release the lock.
   * See PR #299.
   */
...
ChatSession.init({
  ...
  // A fresh heartbeat establishes liveness across processes; ended_at alone cannot distinguish active sessions from
  // orphans. NULL legacy heartbeats are not live; see docs/chat-session-ownership.md.
  lastSeenAt: {
    type: DataTypes.DATE,
    allowNull: true,
  },
...
Tariff.init({
  ...
  // IANA timezone the peak `schedule` is evaluated in (DST-aware), e.g.
  // 'Europe/London'. The call-start instant is converted to this zone's wall clock.
  timezone: { type: DataTypes.STRING, allowNull: false, defaultValue: 'Europe/London' },
  // Per-weekday PEAK window (off-peak = the complement). JSONB:
  //   { mon:{start:'08:00',end:'18:00'}, ..., sat:null, sun:null }
  // A null/absent day = all off-peak; a full-day window (00:00-23:59) = all peak.
  schedule: { type: DataTypes.JSONB, allowNull: false, defaultValue: {} },
  // Flat per-call start cost (micro-pence) assessed on EVERY destination-billed
  // call carried on this tariff, on top of the per-prefix connect fee. May be 0.
  callStartMicros: { type: DataTypes.BIGINT, allowNull: false, defaultValue: 0 },
  // Duration rounding granularity (seconds) — our cost-recovery policy rounds each
  // call's billed duration UP to the next multiple. Stored for frozen-cost
  // reproducibility; 6 = "round up to the next 6-second increment".
  roundingSeconds: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 6 },
 ...
 * digits) priced as a per-call connect fee + a PEAK and an OFF-PEAK per-minute rate
 * (micro-pence; any may be 0; peak==off-peak when a carrier has no time split).
 * Longest-prefix match wins; unique per (tariff, prefix).
 * @class TariffPrefix
 */
...
TariffPrefix.init({
  ...
  // Peak per-minute rate, micro-pence (applies inside the tariff schedule's peak window).
  peakPerMinuteMicros: { type: DataTypes.BIGINT, allowNull: false, defaultValue: 0 },
  // Off-peak per-minute rate, micro-pence (all other times).
  offPeakPerMinuteMicros: { type: DataTypes.BIGINT, allowNull: false, defaultValue: 0 },
  // Minimum CARRIER charge for a call to this prefix, micro-pence — a floor on
  // (connect + per-minute × duration). May be 0.
  minimumMicros: { type: DataTypes.BIGINT, allowNull: false, defaultValue: 0 },
...
Tariff.addHook('beforeUpdate', async (tariff) => {
  const pricingFields = ['name', 'startDate', 'endDate', 'currency', 'defaultCountry', 'timezone', 'schedule', 'callStartMicros', 'roundingSeconds'];
...
// Tariffs intentionally have NO period-overlap constraint. resolveTariff is
// deterministic — the version with the greatest start_date <= billedAt wins — so
// overlapping (or two open-ended) same-name versions are unambiguous: e.g.
// 'x' [1/1, null) and 'x' [2/3, null) mean "x from 1/1, superseded by x from 2/3".
// The unique (name, start_date) index is the only guard (no two versions share a
// start). This DROP removes the earlier EXCLUDE-gist constraint on existing DBs.
async function dropTariffOverlapConstraint() {
  try {
    await sequelize.query('ALTER TABLE "tariffs" DROP CONSTRAINT IF EXISTS tariffs_name_period_excl');
  } catch (err) {
    logger.warn({ err: err?.message }, 'dropTariffOverlapConstraint: ignoring');
  }
}
...
const databaseStarted = sequelize.authenticate()
  ...
  .then(async () => {
    // Ensure chat_sessions outside DB_FORCE_SYNC so deployments without forced schema upgrades still persist history. See
    // PR #148.
    await ChatSession.sync();
    // Plain sync creates missing tables; the gated alter chain must still add columns to existing b2bua_nodes tables. See
    // PR #268. Registry failure is non-fatal because the facade can fall back to discovery.
    try {
...
/**
 * Structural validation of a `bridgedTransferToAgent` map (DTMF sequence →
 * agent reference). Shared between Agent.validate (agent options) and the
 * listener activation path (per-listener override, same shape). Values must
 * already be UUID references — `label:` shortforms are resolved before this
 * runs (agent-sets document save, or listener activation).
 *
 * @param {object} map the candidate map
 * @param {object} flags { hasTransfer, hasAgentTransfer, modelName, context }
 * @throws {Error} (status 400) on any shape violation
 */
function validateBridgedTransferToAgentShape(map, { hasTransfer, hasAgentTransfer, modelName, context = 'options.bridgedTransferToAgent' } = {}) {
  const fail = (message) => {
  ...
  if (typeof map !== 'object' || Array.isArray(map)) {
    fail(`${context} must be an object mapping DTMF sequences to agent references`);
  }
  if (!hasTransfer || !hasAgentTransfer) {
    fail(`Model ${modelName} does not support bridged transfers to agents, but ${context} is set`);
  }
  const entries = Object.entries(map);
  if (!entries.length) {
    fail(`${context} must contain at least one DTMF key`);
  }
  for (const [key, value] of entries) {
    if (!/^[0-9*#]{1,8}$/.test(key)) {
      fail(`${context} key "${key}" must be a DTMF sequence of 1-8 characters from 0-9, * and #`);
    }
    const entry = typeof value === 'string' ? { agent: value } : value;
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
      fail(`${context}["${key}"] must be an agent UUID string or an object with an "agent" property`);
    }
    const allowedFields = ['agent', 'includeHistory', 'fromLabel', 'summaryAgent', 'summaryFromLabel'];
    const unknown = Object.keys(entry).filter((field) => !allowedFields.includes(field));
    if (unknown.length) {
      fail(`${context}["${key}"] has unknown field(s) ${unknown.join(', ')} (allowed: ${allowedFields.join(', ')})`);
    }
    if (!`${entry.agent ?? ''}`.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
      fail(`${context}["${key}"].agent must be an agent UUID (labels are only valid within an agent-set document)`);
    }
    if (entry.includeHistory !== undefined && typeof entry.includeHistory !== 'boolean') {
      fail(`${context}["${key}"].includeHistory must be a boolean`);
    }
    if (entry.fromLabel !== undefined && !/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(`${entry.fromLabel}`)) {
      fail(`${context}["${key}"].fromLabel must be a valid agent-set label`);
    }
    if (entry.summaryAgent !== undefined
      && !`${entry.summaryAgent}`.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
      fail(`${context}["${key}"].summaryAgent must be a text-agent UUID (labels are only valid within an agent-set document or listener activation)`);
    }
    if (entry.summaryFromLabel !== undefined && !/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(`${entry.summaryFromLabel}`)) {
      fail(`${context}["${key}"].summaryFromLabel must be a valid agent-set label`);
    }
  }
}

/**
 * Structural validation of a `bridgedTransferTranscribe` value (boolean or
 * { enabled?, provider?, language? }). Shared between Agent.validate and the
 * listener activation path.
 *
 * @param {boolean|object} transcribe the candidate value
 * @param {object} flags { hasTransfer, modelName, context }
 * @throws {Error} (status 400) on any shape violation
 */
function validateBridgedTransferTranscribeShape(transcribe, { hasTransfer, modelName, context = 'options.bridgedTransferTranscribe' } = {}) {
  const fail = (message) => {
    const err = new Error(message);
    err.status = 400;
    throw err;
  ...
  if (transcribe !== true && transcribe !== false
    ...
    fail(`${context} must be a boolean or an object`);
  }
  if (!hasTransfer) {
    fail(`Model ${modelName} does not support transfers, but ${context} is set`);
  }
  if (typeof transcribe === 'object') {
    ...
    if (transcribe.provider !== undefined && !providers.includes(transcribe.provider)) {
      fail(`${context}.provider must be one of ${providers.map(p => `'${p}'`).join(', ')}`);
    }
