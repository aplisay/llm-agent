...
class Agent extends Model {
  ...
  static async fromNumber(target) {
    ...
    logger.info({ target, number, instance, agent }, 'database got number');
...
class Instance extends Model {
  ...
  async linkNumber(handler, number) {
    ...
    logger.debug({ count, row }, 'update suceeded');
    return row[0].number;
  }
    ...
    hooks: {
      afterCreate: async (call) => {
        if ((await Instance.findByPk(call.instanceId)).streamLog) {
          logger.info(`Streaming logs for call ${call.id}`);
...
class TransactionLog extends Model {
  ...
  static notify(transactionLog, options) {
    ...
    logger.debug({ transactionLog, notify }, `Notifying ${transactionLog.callId}`);
    if (notify) {
      let data;
      try {
      ...
      logger.debug({ [transactionLog.type]: data }, `Notifying logs progress${notify.replace(/-/g, '')}`);
...
TransactionLog.init({
  ...
  type: {
    type: DataTypes.ENUM,
    values: ['start', 'hangup', 'goodbye', 'answer', 'inject', 'call', 'agent', 'user', 'function_call', 'rest_callout', 'function_results'],
    required: true
  },
  data: {
    type: DataTypes.JSONB
...
const databaseStarted = sequelize.authenticate()
  ...
  .then(() => logger.info('Connected to listener'))
  .catch(error =>
