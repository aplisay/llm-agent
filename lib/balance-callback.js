 ...
 *
 * The payload carries the balance twice: `balancePennies` (rounded, for humans
 * and logs) and `balanceMicros` (raw, and the value the `hash` actually signs).
 * Both are needed — a receiver given only the rounded pennies cannot rebuild the
 * canonical string the HMAC was taken over.
 *
  ...
  try {
    ...
    for (const event of events) {
      const payload = {
        ...
        // The signature covers the RAW micro-pence, so the receiver must be sent
        // them: `balancePennies` alone is rounded and cannot reproduce the
        // canonical string. Without this the only way to verify is to scan the
        // ±5000-micro window `Math.round` collapsed — 10001 HMACs per callback
        // on a public, unauthenticated endpoint.
        balanceMicros: nextMicros,
