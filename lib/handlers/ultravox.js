...
class Ultravox extends Handler {
  ...
  async join({ websocket, telephony = false, options = {}, callerId = 'WebRTC', calledId = 'WebRTC' }) {
    let { agent: { id: agentId, userId, organisationId, modelName, options: {fallback: {number: fallbackNumbers} = {} } = {} }, logger, instance: { id: instanceId, metadata: instanceMetadata }} = this;
    const metadata = {
      ...instanceMetadata,
      ...options.metadata,
      aplisay: {
