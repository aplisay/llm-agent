...
class Ultravox extends Handler {
  ...
  async callEnded(ultravoxCall, call) {
    ...
    try {
      ...
      await call.save();
      // Ultravox is a managed, minute-billed realtime service (no exposed token
      //  counts) and this path bypasses Call.end(), so record the voice-minute
      //  usage row here so Ultravox calls still land in the usage ledger.
