...
class Ultravox extends Handler {
  ...
  async join({ websocket, options = {}, callerId = 'WebRTC', calledId = 'WebRTC' }) {
    let { agent: { id: agentId, userId, organisationId, modelName }, logger, instance: { id: instanceId, metadata: instanceMetadata }} = this;
    const metadata = {
      ...instanceMetadata,
      ...options.metadata,
      aplisay: {
    ...
    try {
      ...
      let callId;
      if (!websocket) {
        // If we are a websocket then the caller is responsible for creating the call record
        ({ id: callId } = await Call.create({
          agentId,
          instanceId,
          userId,
          organisationId,
