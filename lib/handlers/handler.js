        ...
        .reduce((o, implementation) => o.concat(
          implementation.allModels.map((row) => {
            ...
            return {
              ...
              // Row-level: a realtime model that can run in text-output mode with
              // an external TTS (see lib/models/pipecat.js EXTERNAL_TTS_PROVIDERS).
              hasExternalTts: flags.externalTts === true,
              // Row-level: the model delegates reasoning and tool use to a backend
              // text agent named by a builtin `delegate` function (docs/gpt-live.md).
              hasDelegation: flags.delegation === true,
              // Row-level: `decision` rows answer a typed result schema in one
              // exchange and have no chat (docs/typesafe-jev.md); everything else
              // is `generative`.
              kind: flags.kind || 'generative',
              description,
              implementation,
