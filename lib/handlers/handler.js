 ...
 */
  ...
  async activate({ number, options = {}, websocket = false } = {}) {
    let { streamLog = false, metadata  } = options;
    ...
    let progressPath;
    logger.debug({ agent, streamLog, options }, `activating agent ${agent.id} with number ${number} or websocket ${websocket}`);
    ...
    let buffer = Buffer.concat([prefix, randomBytes(48)]);
    instance.key = buffer.toString('base64');
    await instance.save();

    let allocated = this.number = number && await instance.linkNumber(type, number, organisationId);
    if (number && !allocated) {
    ...
    return { id: instance.id, number: allocated, key: instance.key, socket: progressPath };
