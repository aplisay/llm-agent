 ...
 * and the builtin `metadata` helper `keys`) are only allowed on handlers that explicitly opt-in.
 *
...
export function validateToolsCallsMetadataUsage({ Handler, functions }) {
  const allowToolsCallsMetadataPaths = !!Handler?.hasDynamicMetadata;
  if (allowToolsCallsMetadataPaths) return;
