...
class Pipecat extends Handler {
  ...
  // The xai/ rows run on the worker's key, but are listed only when this
  // server has an xAI key too, like the text:xai/ rows (docs/grok.md).
  static get availableModels() {
    const rows = super.availableModels;
    return Xai.canLoad.ok ? rows : rows.filter(({ name }) => !name.startsWith(`${this.name}:xai/`));
  }
  ...
  // names are accepted by Pipecat's UltravoxRealtimeLLMService. The xAI block
  // (Grok voice rows, docs/grok.md) comes from the Xai driver's catalogue
  // fetch, which falls back to its static list by itself.
  static voices = (async () => {
    let ultravoxVoices = {};
    try {
    ...
    const xaiVoices = await Xai.voices;
    return {
      ...xaiVoices,
      OpenAI: {
        any: [
