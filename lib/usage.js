 ...
 * @param {Date} [params.billedAt] the billing instant to freeze on the row; when unset it is
 *   resolved at costing (the call's start when the row carries a callId, else the row's creation)
...
export async function recordUsage({
  ...
  // Builtin agent ids are not UUIDs; record their usage without agent_id rather than losing the meter reading. See PR
  // #148.
  ...
  const apply = () => UsageRecord.sequelize.transaction(async (transaction) => {
    ...
    if (billedAt && !record.billedAt) updates.billedAt = billedAt;
...
export async function recordLlmTokens({
  ...
  mode = 'increment', finalised = false, billedAt = null, metadata = null, log = defaultLogger,
} = {}) {
  ...
  for (const [key, unit] of TOKEN_UNITS) {
    ...
    results.push(await recordUsage({
      ...
      quantity, mode, finalised, billedAt, metadata, log,
    }));
  }
  return results;
}
 ...
 * @param {Date} [params.billedAt] freeze the billing instant (an invocation that cites a call
 *   is priced when it runs, not when the call started)
 * @param {object} [params.log]
 * @returns {Promise<Array<UsageRecord|null>>}
 */
export async function recordSubagentUsage({
  sessionId, callId = null, organisationId = null, userId = null,
  usage = [], finalised = false, billedAt = null, log = defaultLogger,
} = {}) {
  const results = [];
  for (const entry of usage || []) {
    ...
    const recorded = await recordLlmTokens({
      ...
      mode: 'increment', finalised, billedAt, log,
    });
