  ...
  async post(body) {
    ...
    try {
      for (let attempt = 1; ; attempt++) {
        ...
        // The vendor's request id on the direct route; OpenRouter sends none and
        // names its generation instead, in the body `id` and a header.
        const headerId = res.headers?.get?.('x-typesafe-request-id') || undefined;
        let text;
        try {
          text = await res.text();
        } catch (err) {
          if (controller.signal.aborted) {
            throw new DecisionRequestError(`decision request exceeded its ${timeoutMs} ms deadline`, { requestId: headerId });
          }
          throw new DecisionRequestError(`decision response could not be read: ${err?.message || err}`, { vendorStatus: res.status, requestId: headerId });
        }
        let json;
        try {
