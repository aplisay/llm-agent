...
/**
 * OpenAI-compatible providers use chat completions; execute bridged MCP tools here rather than returning them as
 * client calls. See PR #149 and docs/mcp-servers.md.
 *
...
class OpenAiCompatible extends Llm {
  ...
  // (openrouter 'moonshotai/kimi-k2.6') — strip only OUR leading provider
  // segment, not everything up to the last slash.
  set model(newModel) {
  ...
  usageOf(u) {
    ...
    return {
      // prompt_tokens includes the cache reads and the cache writes. Report the
      // remainder so the ledger units stay disjoint: a cached or written token
      // must not also be billed at the full input rate.
      inputTokens: Math.max(0, (u?.prompt_tokens || 0) - cacheReadTokens - cacheWriteTokens),
