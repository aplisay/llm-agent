...
class Google extends Llm {
   ...
   */
  async initial() {
    this.logger.info({ tools: this.tools, model: !!this.generativeModel }, 'starting chat with tools');
    ...
    this.chat = this.generativeModel && this.generativeModel.startChat(this.tools || {});
    this.started = true;
  ...
  async rawCompletion(input) {
    ...
    try {
      ...
      this.logger.info({ result, candidates: result.candidates, history: this.chat.history, completion, calls }, 'got response');
    }
    catch (e) {
  ...
  async callResult(results) {
    ...
    let result = await this.rawCompletion(response);
    if (result.error) {
      await new Promise(r => setTimeout(1000, r));
      result = await this.rawCompletion(response);
    }
    this.logger.info({ response, result, gpt: this.gpt }, 'callresponse');
    return result;
  }
}
