...
class Google extends Llm {
  ...
  /**
   * All VertexAI models (probably) support function calling
   *
   * @static
   * @memberof Google
   */
      ...
      functionDeclarations: functions.map(({ name, description, input_schema }) => ({
        ...
        parameters: {
          ...
          properties: Object.fromEntries(
            Object.entries(input_schema.properties).map(([key, {
              type,
              description
  ...
  async rawCompletion(input, callBack) {
    ...
    try {
      ...
      if (um) {
        usage = {
          provider: this.constructor.provider || 'google',
          model: this.model,
    ...
    callBack && callBack({ text: completion, calls, usage });
    return { text: completion, calls, usage };
  }
