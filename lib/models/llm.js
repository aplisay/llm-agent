...
class Llm {
  ...
  /**
   * Whether this model can be given top-level `mcpServers` (remote MCP servers
   * whose tools are exposed to the LLM). Analogous to `supportsFunctions`.
   * Defaults to false; overridden by handlers that can honour MCP (currently
   * the Pipecat worker, which acts as the MCP client).
   *
   * @static supportsMcp
   * @memberof Llm
   */
   ...
   * @memberof Llm
   */
  constructor({ logger, user, prompt, functions, keys, options, modelName }) {
    ...
    // Redacted init log: `keys` carries write-only bearer VALUES (and
    // mcpServers can carry authorization_token) — never serialise them.
    this.logger.debug({
      init: {
        user,
        modelName,
  ...
  async completion(input, callBack) {
    ...
    this.logger.debug({ opts }, 'completion returning');
