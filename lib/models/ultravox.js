...
class Ultravox extends Llm {
  ...
  set functions(functions) {
    ...
    this.tools = functions && functions.map(({ name, key, description, url, method, input_schema: { properties } }) => {
      let pUrl = new URL(url);
      this.logger.debug({ req: {}, url, href: pUrl.href, pUrl, saerchParams: pUrl.searchParams }, `doing function ${name}`);
