...
class OpenAi extends Llm {
  ...
  // Vendor label for usage metering (see lib/usage.js). Subclasses that reuse
  //  the OpenAI-compatible wire format (e.g. Groq) override this.
  ...
  async rawCompletion(input) {
    try {
      ...
      return {
        ...
        usage: u ? {
          provider: this.constructor.provider || 'openai',
