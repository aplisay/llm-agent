...
/**
 * Replay full Responses output with store:false, retaining reasoning and function items.
 * Rewrite hosted MCP results as function pairs so they survive replay; see PR #225 and docs/mcp-servers.md.
 *
 * @class OpenAi
 * @extends {Llm}
 */
class OpenAi extends Llm {
  ...
  async rawCompletion(input, callBack) {
    ...
    const cacheReadTokens = u?.input_tokens_details?.cached_tokens || 0;
    ...
    const result = {
      ...
      usage: {
        ...
        model: this.model,
        // input_tokens includes the cache reads and the cache writes, and each
        // is billed at its own rate instead of the input rate. Report the
        // remainder so the ledger units stay disjoint, as the Anthropic driver
        // does. See https://developers.openai.com/api/docs/guides/prompt-caching
        inputTokens: Math.max(0, (u?.input_tokens || 0) - cacheReadTokens - cacheWriteTokens),
