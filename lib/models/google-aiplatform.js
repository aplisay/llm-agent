...
const {GOOGLE_PROJECT_ID, GOOGLE_PROJECT_LOCATION } = process.env;
...
/**
 * Implements the LLM class for Google's Google model via the AIPlatform
 * interface.
 *
 * @class Google
 * @extends {Llm}
...
class Google extends Llm {
  ...
  static get needKey() {
    return { GOOGLE_PROJECT_ID, GOOGLE_PROJECT_LOCATION };
  }
  ...
  async rawCompletion(input, callBack) {
    ...
    callBack && callBack({ text: prediction?.candidates[0]?.content });
