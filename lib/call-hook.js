...
export function buildCallHookPayload({
  ...
  const base = {
    ...
    agentId,
    listenerId,
    // The owning organisation, the parent leg (null on a root leg) and the model
    // the leg ran on, so a receiver can cross-check its tenant, follow lineage
    // without a lookup, and tell a bridged human leg (`telephony:bridged-call`)
    // from an agent leg. Not covered by the hash, which predates them.
    organisationId: call?.organisationId ?? agent?.organisationId ?? null,
    parentId: call?.parentId ?? null,
    modelName: call?.modelName ?? agent?.modelName ?? null,
