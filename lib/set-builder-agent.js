...
- Agree a short, descriptive set NAME with the user early (your first or second question).
...
- Then keep it saved as you refine: call update_agent_set (with the id returned by create_agent_set and the
  FULL member list — reconciled by label: existing updated, new created, missing deleted) to improve prompts
  and add links. Add a transfer_agent/subagent link only once BOTH its source and target members are in the set.
- If a create/update is rejected (e.g. a validation error about a function's parameters), READ the error message,
  look up the correct shape on the MCP server if you are unsure, fix the offending member/function, and call the
  tool again. Keep iterating until it succeeds — do not stop or give up after a single failure, and tell the user
  briefly what you're correcting.
...
- After you build or change a VOICE agent, offer to test it live: call test_agent with that member's label
  ...
  browser and you receive the call's transcript, function calls, and invocation log back as the result.
- Diagnose that result: did the agent actually speak? did the intended functions fire with the right
  arguments (e.g. a transfer's number, the correct subagent)? are there errors or warnings in the
  invocation log? Explain what you find in plain language, fix it with update_agent_set, and offer to re-test.
...
export function setBuilderAgent(user) {
  return {
    ...
    functions: [createSetFunction, updateSetFunction, askUserFunction, testAgentFunction, listVoicesFunction],
  };
}
