...
distinct terms (e.g. a "British English robotic voice" → search:["british","english","robotic"]). Matches come
back RANKED best-first (most terms hit, each tagged with its matchedTerms and locale) with per-term hit counts —
pick from the TOP of the list and apply it (step 4). If a term appears in unmatchedTerms, that quality genuinely
is not in the catalogue: offer the user the closest real matches instead — that is a normal answer, not a tool
problem, and re-running the same search will not change it. Prefer this over browsing; browse only to explore
what's on offer:
...
- delegate: on a GPT-Live voice agent ("pipecat:openai/gpt-live-1"), names the TEXT agent that is its backend
  brain. The voice agent's prompt is only the persona; the text agent's prompt, model, functions and MCP servers
  do the reasoning and tool calls while the voice agent keeps talking. One per voice agent, only the "agent"
  parameter, target a text member (label:...), and never give the two members a function with the same name.
  A GPT-Live agent with no delegate runs its own prompt and functions as the backend automatically.
...
- mcpServers is honoured by ALL text:… agents (every text provider — anthropic, openai, gemini, kimi,
  openrouter, deepseek, xai) AND by pipecat: voice models. It is NOT honoured by livekit:/jambonz:/ultravox: voice models
...
const listVoicesFunction = {
  ...
  description: 'Look up the REAL TTS voices available for a voice model and its TTS engines. Call with just '
    ...
    + 'description (a big catalogue is trimmed — use `search` to find a specific one). Pass `search` terms to '
    + 'search the whole catalogue: matches come back RANKED best-first (most terms hit), each tagged with its '
    + 'matchedTerms, plus per-term hit counts (termMatches) and any unmatchedTerms. Trust that ranking — the '
    + 'answer to a good search is at the TOP of the list; an unmatched term means the catalogue truly lacks it '
    + '(offer the closest real voice), NOT that the tool failed. Use the returned vendor/voice names verbatim '
    + 'in options.tts — never invent a voice or locale, and never repeat an identical search expecting a '
    + 'different result.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      search: { type: 'array', items: { type: 'string' }, description: 'Optional search terms. Searches name/description/gender/locale across every locale, matching each term at word starts only (case-insensitive; "brit" finds "British" but "male" never matches "female"). Results are RANKED best-first — voices hitting the most terms lead, each tagged with its matchedTerms — with per-term hit counts in termMatches and missing terms in unmatchedTerms (very broad searches are trimmed to the strongest 60 per vendor). Use this to find a voice by accent, language, gender or persona: for "a British English robotic voice" pass ["british","english","robotic"] and pick from the TOP matches. Omit to browse locales/voices instead.' },
    },
    required: ['modelName'],
  },
};
