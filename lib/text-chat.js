...
class TextChatSession {
   ...
   *
   * Each member in the stub also carries its resulting `functions` NAMES. A
   * member's functions are a whole-field replace on save (only keyed functions
   * survive omission — lib/agent-set-functions.js), so a document that rewrites
   * a member can silently drop its transfer wiring; without the names the model
   * has no way to notice, and cannot re-read the set to check. Names only: a
   * handful of tokens per member, against the 2-15KB this exists to avoid.
   */
      ...
      try {
        ...
        return {
          ...
          result: JSON.stringify({
            ...
            label: a.label, id: a.id, name: a.name, functions: functionNames(a.functions),
          })),
          }),
        };
      } catch {
