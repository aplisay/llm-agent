  ...
  openingPrompt() {
    ...
    if (this.seedTestResult && setJson) {
      return 'The user has loaded an agent set and just ran a TEST of one of its agents. '
        + 'Greet very briefly, then analyse the test result below — transcript, function calls, and '
        + 'invocation log: did the agent actually speak? did the intended functions fire with the right '
        + 'arguments? are there errors or warnings in the invocation log? Explain what you find in plain '
        + 'language and propose specific fixes, then apply them with update_agent_set (using the set id) '
