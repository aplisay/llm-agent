...
import { functionNames } from './agent-set-functions.js';
...
/**
 * The option KEYS set on a rendered member, sorted for a stable diff between
 * saves. A fingerprint for the set-save stub (slimResults): names only, so a
 * stored value — an encrypted recording key, a fallback number — never reaches
 * the conversation. Nothing is allowlisted: whatever the row carries is what a
 * save reports.
 */
...
/**
 * The MCP server NAMES on a rendered member, in document order. The companion to
 * optionNames for the set-save stub: an entry's `headers` can hold an inline
 * authorization token and its `url` a credentialed endpoint, so only the name
 * (which namespaces the server's tools at call time) travels.
 */
function mcpServerNames(mcpServers) {
  if (!Array.isArray(mcpServers)) return [];
  return mcpServers
    .map((server) => server && typeof server === 'object' ? server.name : undefined)
...
class TextChatSession {
  ...
  async handleChat(ws) {
    ...
    if (this.ws && this.ws !== ws && this.ws.readyState === this.ws.OPEN) {
      const old = this.ws;
      // `code` lets the superseded client distinguish a takeover from an
      // ordinary drop — it must NOT auto-reconnect (that would steal the
      // session straight back from the tab that just claimed it).
      try { old.send(JSON.stringify({ type: 'error', code: 'superseded', message: 'This session was reconnected elsewhere.' })); } catch { /* dead socket */ }
    ...
    // Protocol-level heartbeat, per socket: proxies/ingress kill a websocket
    // that carries no SERVER traffic for ~60s (nginx proxy_read_timeout
    // default) — exactly what a think-pause, a client-side review run, or the
    // user reading produces. Browsers answer pings automatically, so both
    // directions stay live and the connection stops idle-dropping every
    // minute. A missed pong means the peer vanished without FIN (sleep,
    // killed VM): terminate so 'close' fires and the re-attach grace clock
    // starts now, instead of when TCP finally gives the socket up — until
    // then the session would sit "connected", never tearing down. (Clients
    // may also send app-level {type:"ping"} frames — unknown types fall
    // through the message switch harmlessly.)
    ...
    const heartbeat = setInterval(() => {
      ...
      try { ws.ping(); } catch { /* dying socket — close is imminent */ }
    }, Number(process.env.TEXT_CHAT_HEARTBEAT_MS || 25000));
  ...
  static TEST_ANALYSIS_GUIDANCE =
    ...
    + 'Internal identifiers in the result or the set document (set/agent/call ids) are platform plumbing: a '
    + 'call often runs on a different internal copy of the team than the loaded document, so differing or '
    + 'mismatched ids are EXPECTED and are not findings — match legs to members by label, and never report '
    + 'raw ids or id differences to the user.';
          ...
          if (pause) {
            ...
            if (pausePlatform === 'test_agent') {
              ...
              frame = {
                ...
                options: askOptions(pause.input?.options),
   ...
   *
   * Each member in the stub also carries the NAMES on its resulting `functions`,
   * `options` and `mcpServers`. All three are whole-field replaces on save (only
   * keyed functions survive omission — lib/agent-set-functions.js), so a document
   * that rewrites a member can silently drop its transfer wiring, reduce its
   * options to whatever the save happened to send, or strip a tool server a panel
   * attached; without the names the model has no way to notice, and cannot
   * re-read the set to check. NAMES ONLY, never values: a handful of tokens per
   * member against the 2-15KB this exists to avoid, and nothing the stored values
   * hold (an encrypted recording key, a fallback number, an inline mcpServers
   * authorization header) can ride along.
   */
      ...
      try {
        ...
        return {
          ...
          result: JSON.stringify({
            ...
            label: a.label, id: a.id, name: a.name,
            functions: functionNames(a.functions), options: optionNames(a.options),
            mcpServers: mcpServerNames(a.mcpServers),
          })),
          }),
        };
      } catch {
