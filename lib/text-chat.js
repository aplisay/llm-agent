...
/**
 * Reap by heartbeat age, not process startup, so one server cannot close another's live sessions.
 * Use updated_at for legacy rows and keep cleanup failures non-fatal; see PR #291.
 *
...
/**
 * Report option names only: stored values can contain credentials or phone numbers. See PR #333.
 */
...
/**
 * Report MCP server names only; URLs and headers may contain credentials. See PR #334.
 */
...
class TextChatSession {
  ...
  async handleChat(ws) {
    ...
    // Register socket handlers before awaits so a disconnect during setup still starts grace-period cleanup. See PR #148.
  ...
  openingPrompt() {
    ...
    if (setJson) {
      ...
      if (!Array.isArray(this.latestSet?.agents) || this.latestSet.agents.length === 0) {
        // Keep an existing name when resuming an empty set; the predecessor may already have agreed it with the user. See PR
        // #260.
        const named =
  ...
  async buildLlm() {
    ...
    if (Implementation.kind === 'decision') {
      // Belt and braces behind the route's 400: a decision driver has no completion() to drive a chat with.
      throw new Error(`${this.agent.modelName} is a decision model and has no chat; invoke it with POST /agents/{agentId}/invoke`);
    }
  ...
  /**
   * Keep save stubs to ids and field names: values may contain credentials and full snapshots inflate replay.
   * Report replaced fields so the model can detect omissions; mask failure ids separately. See PRs #333 and #334.
   */
