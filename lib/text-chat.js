...
class TextChatSession {
   ...
   *
   * Each member in the stub also carries its resulting `functions` and `options`
   * NAMES. Both are whole-field replaces on save (only keyed functions survive
   * omission — lib/agent-set-functions.js), so a document that rewrites a member
   * can silently drop its transfer wiring, or reduce its options to whatever the
   * save happened to send; without the names the model has no way to notice, and
   * cannot re-read the set to check. NAMES ONLY, never values: a handful of
   * tokens per member against the 2-15KB this exists to avoid, and nothing a
   * stored option value holds (an encrypted recording key, a fallback number)
   * can ride along.
   */
