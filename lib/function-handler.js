...
const hardwiredBuiltins = {
  ...
  subagent: async (input, metadata, options = {}) => {
    ...
    if (!agent) {
      throw new Error('subagent function call has no agent parameter');
    }
    ...
    if (!base) {
      throw new Error('subagent invocation unavailable: no invoker and no SERVICE_BASE_URI');
    }
  ...
  create_agent_set: async (input, metadata, options = {}) => {
    if (!options.createAgentSet) {
      throw new Error('create_agent_set is not available in this context');
    }
    return await options.createAgentSet(input);
  },
  /** Update an existing agent set (reconciled by label). Needs `options.updateAgentSet`. */
  update_agent_set: async (input, metadata, options = {}) => {
    if (!options.updateAgentSet) {
      throw new Error('update_agent_set is not available in this context');
    }
    ...
    for (const fn of function_calls) {
      ...
      if (f && f.implementation === 'stub') {
        ...
        try {
          ...
          // Return the failure to the model as the tool result (mirroring the
          // `rest` branch) so it can read the message, look up the correct shape
          // on the MCP, fix the call and retry — instead of the error
          // propagating up and aborting the whole turn.
