...
// Cap ranked voice matches at the weakest tail and report unmatched terms so absent voices are not mistaken for tool
// failures. See PR #157.
...
async function functionHandler(function_calls, functions, keys, messageHandler, metadata, specficBuiltins, options = {}) {
  ...
  /**
   * The model-visible copy of a result under a field-list redaction
   * (`redact: ["number", …]`): the JSON result with every property of those
   * names removed, wherever it occurs in the tree (objects and arrays, any
   * depth; exact, case-sensitive names). A result that is not JSON has nothing
   * to strip and is returned as is. The full result still goes to metadata.
   */
  const stripFields = (value, names) => {
    if (Array.isArray(value)) return value.map(v => stripFields(v, names));
    if (value && typeof value === 'object') {
      return Object.fromEntries(
        Object.entries(value).filter(([k]) => !names.has(k)).map(([k, v]) => [k, stripFields(v, names)]),
      );
    }
    return value;
  };
  const redactFields = (result, fields) => {
    const parsed = typeof result === 'string' ? tryParseJson(result) : result;
    if (parsed === undefined || parsed === null || typeof parsed !== 'object') return result;
    const stripped = stripFields(parsed, new Set(fields));
    return typeof result === 'string' ? JSON.stringify(stripped, null, 2) : stripped;
  };
  ...
  if (function_calls) {
    ...
    for (const fn of function_calls) {
      ...
      // `redact: true` hides the whole result behind an acknowledgement;
      // `redact: ["number", …]` hides only the named properties, so one lookup
      // can both disambiguate with the caller and prime a transfer number the
      // model never sees (docs/tool-call-chaining-metadata-priming.md).
      const redaction = !options.allowRedactedFunctionResults ? null
        : f?.redact === true ? 'all'
          : Array.isArray(f?.redact) && f.redact.length ? 'fields'
            : null;
      let result, error;
      ...
      if (f && f.implementation === 'stub') {
        ...
        try {
          ...
          // Abort on infrastructure errors the model cannot repair by retrying. See PR #118.
          if (e instanceof InfrastructureError) throw e;
          // Return other errors as tool results so the model can correct its arguments. See PR #118.
      ...
      const llmResult = redaction === 'all'
        ? (error ? 'FAILED - invocation failed' : 'OK - function completed')
        : redaction === 'fields'
          ? (error ? 'FAILED - invocation failed' : redactFields(rawResult, f.redact))
          : rawResult;
