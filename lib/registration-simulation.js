// The simulator is disabled by default in production now that real registration is wired up.
// It remains available for standalone testing and in unit tests.
const SIM_ENABLED = process.env.REG_SIM_ENABLED === 'true' || process.env.NODE_ENV === 'test';
      ...
      const finalTimeout = setTimeout(async () => {
        ...
        const flipTimeout = setTimeout(async () => {
          if (Math.random() < 0.5) { // 50% chance to flip
            const { PhoneRegistration } = await import('./database.js');
