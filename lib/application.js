...
const Gpt35 = require('./models/gpt35');
const Gpt4 = require('./models/gpt4');
...
class Application {
   ...
   *  
   */
  constructor({ modelName, wsServer, makeService, options, logger, name, prompt, callbackUrl }) {
    if (!Application.agents[modelName]) {
      throw new Error(`Bad agent name ${modelName} must be one of ${Object.keys(Application.agents)}`);
    }
    logger.info({ wsServer }, 'Application create wsServer WS');
    Object.assign(this, { modelName, wsServer, makeService, logger, options, prompt, callbackUrl });
    this._prompt = prompt || Application.agents[modelName].defaultPrompt;
    this._options = options;
    this.name = name || uuid();
    this.id = `LLM-${this.modelName}-${this.name}`;
   ...
   */
  static listModels() {
    return Object.fromEntries(Object.entries(Application.agents)
      .filter(([, value]) => !!value.implementation)
      .map(([key, { description, defaultPrompt }]) => ([key, { description, defaultPrompt }])));
  }
  ...
  async create() {
    try {
      this.agent = new Agent({
        ...
        llmClass: Application.agents[this.modelName].implementation,
