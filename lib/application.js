...
class Application {
  ...
  async create() {
    ...
    let { prompt, options, functions, logger } = this;
    ...
    try {
      ...
      if (!llmClass.audioModel) {
        ...
        this.logger.info({ name: this.name }, 'creating inband application');
        this.agent = new llmClass({ prompt, options, functions, logger });
        return await this.agent.startInband();
      }
    }
    catch (e) {
