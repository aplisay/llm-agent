...
class Application {
   ...
   * @param {*} param0 
   */
  constructor({ agentName, makeService, options, logger, name, prompt }) {
    if (!Application.agents[agentName]) {
      throw new Error(`Bad agent name ${agentName} must be one of ${Object.keys(agents)}`);
    }
    Object.assign(this, { agentName, makeService, logger, options, prompt });
    this._prompt = prompt || agents[agentName].defaultPrompt;
    this.name = name || uuid();
  ...
  async create() {
    ...
    this.agent = agent({ ...this, prompt: this.prompt, llmClass: Application.agents[this.agentName].implementation });
