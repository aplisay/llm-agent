...
class Application {
   ...
   *  
   */
  constructor({ modelName, wsServer, makeService, options, logger, name, prompt }) {
    ...
    Object.assign(this, { modelName, wsServer, makeService, logger, options, prompt });
   ...
   */
  static listModels() {
    return Object.fromEntries(Object.entries(Application.agents).map(([key, value]) => ([key, {...value, implementation: null}])));
  }
    ...
    await Application.#allocateMutex.runExclusive(async () => {
      ...
      for (let tries = 0; !this.number && tries < 3; tries++) {
        ...
        try {
          ...
          ({ candidate, application: this.application}, 'Error');
          this.logger.error({ candidate, application: this.application }, 'Error');
        }
      }
    });
