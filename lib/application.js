...
class Application {
   ...
   *  
   */
  constructor({ modelName, wsServer, makeService, options, logger, name, prompt, functions, callbackUrl }) {
    ...
    if (functions && !Application.agents[implementationName]?.implementation?.supportsFunctions){ 
    ...
    Object.assign(this, { implementationName, model, wsServer, makeService, logger, options, prompt, functions, callbackUrl });
  ...
  static listModels() {
    ...
    return Object.fromEntries(agentImplementations
      .map(([key, { implementation, defaultPrompt }]) => (
        implementation?.allModels?.map(
          ([model, description]) => ([`${key}:${model}`, {
            description,
            defaultPrompt,
            supportsFunctions: implementation.supportsFunctions
          }])
        )
      ))
      .flat());
  ...
  async create() {
    try {
      this.agent = new Agent({
        ...
        llmClass: Application.agents[this.implementationName].implementation,
