...
import { resolveTariff, matchTariffPrefix, destinationCostMicros } from './tariffs.js';
 ...
 * → a per-call connect fee PLUS a per-minute rate over the row's minutes. Additive
 * with the exact-dimension cost. Returns null when the card has no destination
 * line, the row isn't destination-billable, or no tariff/prefix matches.
 *
...
export async function resolveDestinationCost(row, card, billedAt, {
  ...
  const number = row?.metadata?.destination;
  if (!destLine || !number) return null;
  ...
  const minutes = toLineUnits(row.quantity, row.unit, 'minute');
  const connectMicros = Number(prefixRow.connectMicros) || 0;
  const perMinuteMicros = Number(prefixRow.perMinuteMicros) || 0;
  const costMicros = destinationCostMicros(prefixRow, minutes);
  const startIso = (tariff.startDate instanceof Date ? tariff.startDate : new Date(tariff.startDate)).toISOString();
  return {
    costMicros,
    entry: {
      dim: 'destination', tariff: tariff.name, tariffStart: startIso, prefix: prefixRow.prefix,
      connectMicros, perMinuteMicros, minutes, costMicros,
    },
  };
}
