 ...
 * left uncosted. Stored as a NAME (not an id/flag) because rate cards are
 * temporal + immutable: an org's rate is always keyed by name and resolved to
 * the covering version at cost time. Absent/empty value = "no platform default"
 * (the pre-existing untracked behaviour). Singleton — mirrors `dbVersion`.
 */
export const DEFAULT_RATE_KEY = 'defaultRateName';

/**
 * The platform-wide default rate NAME, or null when none is configured.
 * @returns {Promise<string|null>}
 */
export async function getDefaultRateName({ Metadata = DefaultMetadata } = {}) {
  const row = await Metadata.findOne({ where: { key: DEFAULT_RATE_KEY } });
  const value = row?.value;
  return (typeof value === 'string' && value) ? value : null;
}

/**
 * Validate a candidate platform-default rate name. null / '' is VALID (clears the
 * default). A non-empty name must have a rate card COVERING `now` — so we never
 * pin the platform to a name that would resolve to nothing. Mirrors the
 * error-string-or-null contract of {@link validateRateHistory}.
 * @returns {Promise<string|null>} an error message, or null when valid.
 */
export async function validateDefaultRateName(name, { RateCard = DefaultRateCard, now = new Date() } = {}) {
  if (name == null || name === '') return null; // clear the default
  if (typeof name !== 'string') return 'defaultRateName must be a string or null';
  const card = await resolveRateCard(name, now, { RateCard });
  if (!card) return `no rate card "${name}" covers ${now.toISOString()}`;
  return null;
}

/**
 * Set (or, with null/'' , clear) the platform-wide default rate name. Idempotent
 * upsert of the {@link DEFAULT_RATE_KEY} singleton — does NOT validate; callers
 * should run {@link validateDefaultRateName} first (the API route does).
 * @returns {Promise<string|null>} the stored name (null when cleared).
 */
export async function setDefaultRateName(name, { Metadata = DefaultMetadata } = {}) {
  const value = (typeof name === 'string' && name) ? name : null;
  await Metadata.upsert({ key: DEFAULT_RATE_KEY, value });
  return value;
}

/**
 * The rate-history entry a NEW organisation should start on: the platform
 * default, as a single `[{ name, startDate: now }]` timeline. Returns null when
 * no default is configured OR the configured name has no card covering `now`
 * (defensive: never stamp an org with a name that would fail validation / resolve
 * to nothing — fall back to the untracked/uncosted default instead). Used by both
 * org-creation paths (admin create + self-signup provisional org).
 * @returns {Promise<Array<{name:string, startDate:string}>|null>}
 */
export async function defaultRateHistoryEntry({ Metadata = DefaultMetadata, RateCard = DefaultRateCard, now = new Date() } = {}) {
  const name = await getDefaultRateName({ Metadata });
  if (!name) return null;
  const card = await resolveRateCard(name, now, { RateCard });
  if (!card) return null;
  return [{ name, startDate: now.toISOString() }];
}
