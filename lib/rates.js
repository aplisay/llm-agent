...
/** The four orthogonal pricing dimensions; a card line declares exactly one. */
export const DIMENSIONS = ['audio-path', 'model', 'tts', 'stt'];
...
export async function settle(row, org, { transaction } = {}) {
  if (!org || org.balance === null || org.balance === undefined) return; // untracked
  if (row.costMicros === null || row.costMicros === undefined) return;   // nothing to apply
...
export async function costUsageRow(row, {
  ...
  try {
    ...
    const org = row.organisationId
      ...
      : null;
    const rateName = resolveOrgRateName(org, billedAt);
    ...
    const { costMicros, status, breakdown } = resolveRowCost(row, card);
    await row.update({
      billedAt,
      costMicros,
