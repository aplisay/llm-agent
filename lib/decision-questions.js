...
export const RESERVED_RESULT_NAMES = ['confidence', 'probabilities', 'score', 'decision'];
 ...
 * legend, so the keys are always the declared names; `score` carries the
 * vendor's expected value (the probability-weighted level index) per Score.
 *
...
export function resultFromAnswers(answers, questions, decision = undefined) {
  ...
  for (const [name, question] of Object.entries(questions)) {
    ...
    if (question.type === 'choice') {
      ...
      score[name] = numberOr(name, 'score', answer.score);
    }
