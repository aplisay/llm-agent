...
function describeShape(p, type) {
  if (type === 'string') return 'a free string';
  if (['number', 'integer', 'array', 'object'].includes(type)) return `type "${type}"`;
...
function choiceCriteria(name, options, descriptions) {
  if (descriptions !== undefined && descriptions !== null) {
    ...
    for (const [key, text] of Object.entries(descriptions)) {
      ...
      if (text !== null && typeof text !== 'string') {
        throw new DecisionSchemaError(`${name}: x-descriptions["${key}"] must be a string or null`);
      }
    }
  }
  ...
  return Object.fromEntries(options.map((option) => [option, descriptions?.[option] ?? null]));
}
...
export function questionFromProperty(name, property) {
  ...
  throw new DecisionSchemaError(
    `${name}: a decision model can answer an enum (Choice), a boolean (Noul) or x-levels (Score); ${describeShape(property, type)} is not answerable`);
}
...
function distribution(name, value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new DecisionAnswerError(`the decision model returned no probabilities for "${name}"`);
  }
  return Object.fromEntries(Object.entries(value).map(([key, p]) => [key, numberOr(name, `probability of "${key}"`, p)]));
}

/**
 * The invocation result for a set of answers (docs/typesafe-jev.md, "The
 * result"): each property carries its typed answer, `confidence` and
 * `probabilities` carry the Choice and Score detail keyed by property, and
 * `decision` is present only when `minConfidence` is configured.
 *
 * Choice: the value is the chosen option. Score: the value is the most
 * probable level name (the first on a tie), the distribution re-keyed from
 * level index to level name. Noul: the value is the probability of yes, and
 * the property has no confidence or probabilities entry.
 *
...
export function resultFromAnswers(answers, questions, decision = undefined) {
  ...
  for (const [name, question] of Object.entries(questions)) {
    ...
    if (question.type === 'choice') {
      ...
      probabilities[name] = distribution(name, answer.probabilities);
    } else {
      const levels = question.criteria;
      const legend = answer.legend && typeof answer.legend === 'object' ? answer.legend : {};
      const byIndex = distribution(name, answer.probabilities);
      const byName = {};
      let best = null;
      let bestProbability = -Infinity;
      levels.forEach((level, index) => {
        const key = String(index);
        const levelName = typeof legend[key] === 'string' ? legend[key] : level;
        const p = byIndex[key];
        if (p === undefined) {
          throw new DecisionAnswerError(`the decision model returned no probability for level ${index} ("${level}") of "${name}"`);
        }
        byName[levelName] = p;
