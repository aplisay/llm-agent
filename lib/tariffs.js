...
export function computeDestinationCost(tariff, prefixRow, { billedAt, durationMs } = {}) {
  ...
  const costMicros = Math.round(callStartMicros + connectMicros + perMinuteMicros * minutes);
  return { costMicros, peak, perMinuteMicros, connectMicros, callStartMicros, roundingSeconds, billedSeconds, minutes };
}
...
export function validatePrefixes(prefixes) {
  ...
  for (let i = 0; i < prefixes.length; i += 1) {
    ...
    seen.add(p.prefix);
    for (const f of ['connectMicros', 'peakPerMinuteMicros', 'offPeakPerMinuteMicros']) {
      if (!Number.isFinite(Number(p[f])) || Number(p[f]) < 0) return `prefix ${i}: ${f} must be a number >= 0`;
    }
  }
  return null;
}
