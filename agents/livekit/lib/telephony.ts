...
export async function transferParticipant(
  ...
  logger.info(
    { roomName, participant, transferTo, registrar, transport },
  ...
  if (registrar) {
    ...
    const registrarHost = (registrar as string)
      .replace(/^sip:/i, "")
      .replace(/^tel:/i, "");
  ...
  const result = await sipClient.transferSipParticipant(
    roomName,
    participant,
    transferUri,
    sipTransferOptions,
 ...
 */
function mapTransportToSIPTransport(
...
async function findOrCreateRegistrationTrunk(
  ...
  let registrationTrunk = outboundSipTrunks.find((t) => t.name === trunkName);

  if (registrationTrunk) {
    logger.info(
      { trunkName, sipTrunkId: registrationTrunk.sipTrunkId },
      "Found existing registration trunk",
    );
  ...
  logger.info(
    { trunkName, registrarHost, b2buaAddress, transport },
    "Creating new registration trunk",
  );
...
export async function bridgeParticipant(
  ...
  if (registrationOriginated && b2buaGatewayIp && registrationEndpointId) {
    ...
    logger.info(
      ...
      "bridge participant initiated (registration endpoint)",
    );
    ...
    logger.info(
      { newParticipant, registrationTrunkId },
      "new participant result (registration endpoint)",
    );
  ...
  if (!aplisayId?.length) {
    throw new Error(
      "No inbound trunk or inbound trunk does not support bridging",
    );
  }
  ...
  logger.info(
    { roomName, destination, origin, callerId, sipParticipantOptions },
    "bridge participant initiated (trunk-based)",
  );
...
export async function dialTransferTargetToConsultation(
  ...
  if (registrationOriginated && b2buaGatewayIp && registrationEndpointId) {
    ...
    logger.info(
      {
        transferTargetParticipant,
        consultRoomName,
        destinationFormatted,
        registrationEndpointId,
        registrationTrunkId,
      },
      "transfer target dialed through registrar trunk with registration endpoint ID",
    );
  ...
  logger.info(
    { transferTargetParticipant, consultRoomName, destination },
    "transfer target dialed into consultation room",
  );
  return transferTargetParticipant;
}
