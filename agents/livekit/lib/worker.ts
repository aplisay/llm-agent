    ...
    try {
      ...
      if (outbound) {
        ...
        if (!instance) {
          ...
          logger.info(
            { callerId, calledId, instanceId, aplisayId, outbound },
        ...
        if (!participant) {
          logger.error(
            { ctx },
            `Outbound call failed for (${calledId} => ${callerId})`
          );
      ...
      logger.info(
        { agent, instance, calledId, callerId, ctx, room, participant },
        "new room instance"
      );
      ...
      const transfer = async (args: any) => {
        ...
        try {
          ...
          bridgedParticipant = await bridgeParticipant(
            room.name,
            args.number,
            aplisayId,
            calledId
      ...
      if (!realtime) {
        logger.error(
          { modelName, plugin, realtime, models },
          "Unsupported model"
        );
      ...
      session.on("response_output_added", (newOutput: any) =>
        logger.debug({ newOutput })
      );
      ...
      ctx.room.on("participantDisconnected", async (p: any) => {
        ...
        // Depending on how the call started we may have an agents participant object or a bridged SIP participant object
        if (
          p.info.sid === (participant?.info?.sid || participant?.participantId)
        ) {
          logger.info(
            { participant },
            "Original participant disconnected, closing realtime model"
          );
          model && (await model.close()) && (model = null);
          call.end();
          sendMessage({ hangup: `${calledId} => ${callerId}` });
        ...
        if (p.info.sid === bridgedParticipant?.participantId) {
          logger.info(
            { bridgedParticipant },
            "Bridged participant disconnected, closing whole room"
          );
          room && room.name && (await roomService.deleteRoom(room.name));
