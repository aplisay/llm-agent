# Release 0.9.55 - GPT-Live and Grok, SIP registrar accounts and observability, auxiliary transcription, worker hardening
...
- **[livekit] Greeting and inactivity prompt** now play on realtime sessions
  with no TTS, such as OpenAI Realtime in its own voice.
- **[livekit] Gemini Live model lookup** now finds the model in the Google
  plugin, so `livekit:google/` realtime sessions no longer fail at start with
  `Unsupported realtime model`.
- **[pipecat] Tool result cap**: MCP and REST function results shown to the
  model are cut at 8000 bytes, or 2500 bytes on a GPT-Live responses
  delegation, with a note asking for a smaller part. `metadata.toolsCalls`
  keeps the full value for chaining.

## Agent handover and hand-back - livekit + pipecat

- **[livekit + pipecat] Handover opening**: after a `transfer_agent` handover,
  in place or full-stack, the incoming agent skips its greeting. It speaks
  first, introduces itself, and continues from the handover summary and the
  conversation so far.
- **[livekit + pipecat] Hand-back opening**: after a person hands a call back
  to an agent (`bridgedTransferToAgent`), the incoming agent skips its greeting
  and continues from what the caller discussed.
- **[livekit + pipecat] Interruptible openings**: the caller can interrupt the
  incoming agent's first turn after a handover or hand-back. On Pipecat the
  greeting mute, including GPT-Live's, now applies only when a greeting plays.
- **[livekit] Opening transcripts**: on LiveKit pipeline stacks, the
  instruction that starts the incoming agent's first turn is not logged as the
  caller's words.
- **[livekit] Inactivity after a handover**: `options.inactivity` prompts now
  follow the active agent on every session of a call, including Ultravox
  in-place handovers, so the incoming agent's message, timeout, and hangup rule
  apply.
- **[livekit] Ultravox handover legs**: when Ultravox ends a session after a
  handover or hand-back, the call now ends instead of leaving the caller in
  silence.
...
- **[core] GPT-Live voices** are listed under `OpenAI` for the row (default
  `marin`). `options.tts.vendor` must be unset or `openai`, except with the
  experimental transcript TTS below. Each voice has a `gender` and a
  `description` naming its accent, so `list_voices` can search by accent.
- **[pipecat] Experimental transcript TTS**: with
  `options.tts.experimentalTranscript: true` and an external
  `options.tts.vendor`, the worker discards GPT-Live's own audio and speaks its
  output transcript through that TTS. Off by default.
...
- **[pipecat] Agent handover** (`transfer_agent`) on GPT-Live is always a full
  restart.
- **[pipecat] Delegation recovery**: when the OpenAI backend refuses a tool
  result because the session's input budget is full, the worker answers it with
  a short placeholder so the response continues. If no backend answer arrives
  within 4 s, the voice model apologises to the caller.
- **[core] Documentation**: new [gpt-live.md](../gpt-live.md) and
  [gpt-live-agent-sets.md](../gpt-live-agent-sets.md).

## xAI Grok - core + pipecat

- **[core + pipecat] Availability**: the Grok voice and pipeline rows run on the
  Pipecat worker only. The API server and the worker need `XAI_API_KEY`
  (`GROK_API_KEY` is accepted as a fallback); without it the rows are not
  advertised.
- **[core + pipecat] `pipecat:xai/grok-voice-think-fast-2.0`** adds xAI's Grok
  voice model as a realtime row, with voices from xAI's catalogue.
  `greeting.text` and the `options.inactivity` message are spoken word for word
  and cannot be talked over.
- **[core] Grok text models**: `text:xai/grok-4.6`, `text:xai/grok-4.3`,
  `text:xai/grok-4.20-0309-reasoning`, and
  `text:xai/grok-4.20-0309-non-reasoning`. `options.effort` maps to each
  model's reasoning effort.
- **[core + pipecat] Grok pipeline rows**: `pipecat:xai/grok-4.3` and
  `pipecat:xai/grok-4.20-0309-non-reasoning` run as the LLM stage of a Pipecat
  pipeline.
- **[core] Server-side tools**: on the Grok voice row, a function named after
  one of xAI's server-side tools is rejected at save time, as is a `tools`
  array or a server-tool entry in `vendorSpecific.xai.session`.
- **[pipecat] `vendorSpecific.xai.session`** merges into the voice session
  configuration.
- **[core] Billing**: the voice row is priced per minute, and its own speech on
  the zero `tts:xai` line. Grok text models are priced on token lines.
- **[core] Documentation**: new [grok.md](../grok.md).
...
- **[core] Bundled realtime speech**: `/api/rate-components` lists
  `tts:ultravox`, `tts:openai`, and `tts:xai` with `bundled: true`, so a rate
  card can price speech already charged by the model at zero.
- **[pipecat] Realtime speech metering**: a realtime model speaking in its own
  voice is metered under the model's vendor (`ultravox`, `openai`, `xai`)
  instead of the default pipeline TTS vendor. Gemini Live speech is not
  metered.
- **[core] `scripts/add-xai-rate-lines.mjs`** adds Grok voice and text lines,
  and a zero `tts` line for each bundled provider, to every card that prices
  models. A second run changes nothing.
...
- **[core] Hand-off Google sign-in**: `/api/oauth-handoff/start` allows a new
  account only with `?signup=1`. Without it, a Google account with no user here
  is refused, with `?error=signup_disabled` on the error callback.
...
- **[core] Tool failure messages** reaching the model now omit internal set,
  agent, and draft ids.
- **[core] `ask_user` options** are sent to clients as strings, as the tool
  declares, even when the model supplies objects.
- **[core] Set-save results** list each member's function names, option keys,
  and MCP server names, so a builder session can check what a save kept.
...
- **[pipecat] Pipecat 1.10.0**: the worker moves from pipecat-ai 1.6.0 to
  1.10.0, installed from PyPI.
- **[pipecat] MCP client** moves to the mcp 2 SDK. Agent `mcpServers`
  configuration, including `transport: "streamable_http"`, is unchanged.
- **[core + livekit + pipecat + sipbridge] Dependency security updates**: npm
  and Python lockfiles are patched against open advisories, including nltk
  3.10.3 in the Pipecat worker. sipbridge builds on Go 1.26 with
...
- **[core] New optional environment**: `REGCLIENT_API_TOKEN`,
  ...
  `TEXT_CHAT_LIVENESS_GRACE_MS`, `TEXT_CHAT_STATE_MAX_BYTES`, `POD_NAME`, and
  `XAI_API_KEY`.
- **[pipecat] New optional environment**: `WEBRTC_OUTPUT_CUSHION_MS`,
  ...
  `HEALTHZ_MAX_TASKS`, `HEALTHZ_MAX_GATEWAY_ENTRIES`, and `XAI_API_KEY`.
...
- **[core] `scripts/add-xai-rate-lines.mjs`** must run on each environment.
  Without its zero `tts` lines, realtime speech rows from the Pipecat worker
  settle as `no_line`. A card with no Ultravox minute line needs
  `XAI_VOICE_PRICE_MICROS`.
- **[core] Hand-off sign-in**: a client that starts a Google sign-in through
  `/api/oauth-handoff/start` and needs it to create new accounts must pass
  `?signup=1`.
- **[pipecat] Tool results** over 8000 bytes (2500 on a GPT-Live responses
  delegation) are now cut before the model sees them. Agents that depend on
  larger MCP or REST results need narrower endpoints.
