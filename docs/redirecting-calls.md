...
Metadata dot-notation supports arbitrary depth, including values primed from earlier tool calls and stored at `metadata.toolsCalls.<toolName>.parameter` or `metadata.toolsCalls.<toolName>.result` (for JSON tool results).
...
### Customizing the Transfer Prompt
...
**Note:** The `transferPrompt` parameter only applies to consultative transfers (`operation: "consultative"`). It is ignored for blind transfers.
...
6. The `transferPrompt` parameter (when used in function calls) can only be specified as `static` - it cannot be generated by the LLM or sourced from metadata
