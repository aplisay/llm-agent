...
The tone is generated by the worker and is supported on both the LiveKit and Pipecat stacks for all transfer mechanisms (bridging, SIP REFER, and the WebRTC worker-side media relay). When `transferTone` is unset, behaviour is unchanged and no tone path is constructed.
