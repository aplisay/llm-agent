...
The `transfer.number` parameter must be `static` or `metadata` (never `generated`). Because `metadata` supports arbitrary-depth dot paths, you can also source `transfer.number` from values written into `metadata.toolsCalls` by earlier tool calls (server-side tool result chaining). See [`tool-call-chaining-metadata-priming.md`](./tool-call-chaining-metadata-priming.md) for a DB-backed receptionist example.
