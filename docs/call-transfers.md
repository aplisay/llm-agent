...
> **Note — the comfort tone is on the wire but not in the call recording.** On the LiveKit stack the tone is published as a *separate* audio track on the agent's room participant, whereas the recording is captured per-participant from the agent session's own input (caller) and output (agent) — two channels that do not include extra published tracks. So the caller hears the tone, but it will **not** appear in the saved recording, and any QA/eval that inspects the recording rather than the delivered caller audio will see silence (at most a brief echo blip before the far-end echo canceller settles). The same blind spot would apply to any other separately-published audio (e.g. background/hold music). If a transfer-tone needs to be verified, check the caller leg's received audio or a room-composite recording, not the per-participant capture. The tone frequencies (`523`/`587`/`659` Hz for `low`/`medium`/`high`) deliberately avoid the UK/EU network-tone frequency (425 Hz) and the DTMF bands so the caller doesn't mistake it for a dial/busy tone.
...
- **The incoming agent's first turn.** On Pipecat the agent's greeting is not used, because the caller was greeted when the call started. The platform tells the agent to introduce itself in one sentence and continue from what the caller discussed (or ask how it can help, when it has no history). The caller can interrupt that turn. The LiveKit worker asks the incoming agent to greet the caller according to its instructions.
